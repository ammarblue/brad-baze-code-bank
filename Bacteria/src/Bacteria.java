 ...
 */
public class Bacteria extends PApplet {
	int SIZE = 300;
	int MAX_AGE = 1024;
	...
	class Bact {
		...
		// random position
		Bact() {
			this((int) (random(0, SIZE)), (int) (random(0, SIZE)));
		}
		...
		public synchronized Bact live(Iterator it, Bact[][] sexMap) {
			...
			if (age < MAX_AGE) {
				...
				// have sex if we are mature and there is a partner of the other
				// sex
				if (isMature()) {
					if ((sexMap[x][y] != null)
							&& (sexMap[x][y].male != this.male)) {
