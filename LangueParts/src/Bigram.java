...
class Bigram {
	...
	public Bigram(TagSet tags) {
		...
		if (SentenceBoundaryIndex == -1) {
			...
			System.out
					.println("This will probably cause an ArrayIndexOutOfBounds error.");
		}
	}
	...
	public int[] Viterbi(Word[] words) {
		...
		for (int word_id = 1; word_id < words.length; ++word_id) {
			...
			int from_tag = 0;
			while (0 != (from_tag = Tags.GetNextTagFromBitField(from_tags,
					from_tag))) {
				int to_tag = 0;
				while (0 != (to_tag = Tags.GetNextTagFromBitField(to_tags,
						to_tag))) {
					float log_prob = probabilities[word_id - 1][from_tag]
							+ LogProb[from_tag][to_tag];
		...
		try {
			...
			while ((line = buffer.readLine()) != null) {
				...
				if (tokens.length > column_of_tags) {
					...
					int tag_index = Tags
							.GetIndexFromString(tokens[column_of_tags]);
					if (tag_index != -1
							&& preceding_tag_index != -1
							&& (tag_index != SentenceBoundaryIndex || preceding_tag_index != SentenceBoundaryIndex)) {
	...
	private void RefreshLogProbs() {
		...
		for (int row = 0; row < Tags.Size(); ++row) {
			...
			for (int col = 0; col < Tags.Size(); ++col) {
				LogProb[row][col] = (float) Math
						.log((Counts[row][col] + smoothing) / row_sum);
			}
		}
	}
}
