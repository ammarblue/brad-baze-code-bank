...
public class Console extends PApplet {
	...
	static private int FontSize = 12;
	...
	static void Initialise(PApplet applet, Application app, PFont font,
			int font_size) {
		Applet = applet;
		App = app;
		Font = font;
	...
	static void Write(String s) {
		...
		for (int c = 0; c < s.length(); ++c) {
			...
			if (Applet.textWidth(line.toString()) > (max_line_width - max_char_width)
					|| s.charAt(c) == '\n') {
	...
	static void KeyPressed() {
		...
		case DELETE: {
			String before_cursor = CurrentLine.substring(0,
					Math.max(CaratPosition, 0));
			String after_cursor = CurrentLine.substring(
					Math.min(CaratPosition + 1, CurrentLine.length()),
					CurrentLine.length());
		...
		case BACKSPACE: {
			String before_cursor = CurrentLine.substring(0,
					Math.max(CaratPosition - 1, 0));
			String after_cursor = CurrentLine.substring(
					Math.min(CaratPosition, CurrentLine.length()),
					CurrentLine.length());
			CurrentLine = before_cursor + after_cursor;
			--CaratPosition;
			break;
		...
		case 0x16: // Paste
		{
			Clipboard clipboard = Applet.getToolkit().getSystemClipboard();
			...
			String after_cursor = CurrentLine.substring(
					Math.min(CaratPosition, CurrentLine.length()),
					CurrentLine.length());
			CurrentLine = before_cursor + text + after_cursor;
	...
	static void Draw() {
		...
		// Draw current line:
		int right_aligned = Applet.width - MarginRight
				- (int) Applet.textWidth(Prompt + CurrentLine + Carat);
