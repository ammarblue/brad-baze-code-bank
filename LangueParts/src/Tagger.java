...
public class Tagger extends PApplet implements Console.Application {
	...
	public Tagger() {
		InputStream tag_input_stream = getClass().getResourceAsStream(
				"data/TagSet.txt");
		InputStream dict_input_stream = getClass().getResourceAsStream(
				"data/Dictionary.txt");
		InputStream bigram_input_stream = getClass().getResourceAsStream(
				"data/Bigram.txt");
		if (tag_input_stream != null && dict_input_stream != null
				&& bigram_input_stream != null) {
			InputStreamReader tag_input_isr = new InputStreamReader(
					tag_input_stream);
			InputStreamReader dict_input_isr = new InputStreamReader(
					dict_input_stream);
			InputStreamReader bigram_input_isr = new InputStreamReader(
					bigram_input_stream);
			...
			try {
				Reader tag_reader = new FileReader(sketchPath
						+ "/Data/TagSet.txt");
				Reader dict_reader = new FileReader(sketchPath
						+ "/Data/Dictionary.txt");
				Reader bigram_reader = new FileReader(sketchPath
						+ "/Data/Bigram.txt");
	...
	public long GuessPennTags(String word) {
		...
		if (word.endsWith("us") || word.endsWith("ic") || word.endsWith("ble")
				|| word.endsWith("ive") || word.endsWith("ary")
				|| word.endsWith("ful") || word.endsWith("ical")
				|| word.endsWith("less")) {
		...
		if (word.endsWith("ly"))
			bit_field |= Tags.GetBitsFromString("RB");
		...
		if (bit_field == 0)
			bit_field |= Tags.GetBitsFromString("NN");
	...
	public String TabComplete(String s) {
		...
		String[] strings_that_pass = {
				...
				"My dog also likes eating sausage.", "He saw the duck.",
				"He used the saw.", "Unknown wurds handled spiffingly.",
				"Carl's tagger don't got no problem with apostrophes.",
				"Flying planes can be dangerous." };

		String[] strings_that_fail = { "Time flies like an arrow.", // "Flies"
																	// should be
																	// VBZ, not
																	// NNS.
				"The man whistling tunes pianos.", // "Tunes" should be VBZ, not
													// NNS.
				"They were too close to the door to close it.", // First "close"
																// should be JJ,
																// not VB.
				"The insurance was invalid for the invalid.", // First "invalid"
																// should be JJ,
																// not NN.
				"When shot at, the dove dove into the bushes.", // Second "dove"
																// should be
																// VBD, not NN.
		};
