...
public class Client implements Runnable {
    ...
    while (Thread.currentThread() == thread) {
      try {
        ...
        if (clientEventMethod != null) {
		  ...
		  //CHANGES BY BRAD BAZEMORE
		  //ANSWER YES THIS IS BAD
		  //REMOVING
		  //END CHANGES BY BRAD BAZEMORE
          Thread.sleep(10);
  ...
  public int read() {
	...
	public float readFloat(){
		...
		synchronized(buffer){
			int out=buffer[bufferIndex++];
			float outgoing=Float.intBitsToFloat(out);
			if(bufferIndex==bufferLast){
				bufferIndex=0;
				bufferLast=0;
			}
	...
	public double readDouble(){
		...
		synchronized(buffer){
			int out=buffer[bufferIndex++];
			double outgoing=Double.longBitsToDouble(out);
			if(bufferIndex==bufferLast){
				bufferIndex=0;
				bufferLast=0;
			}
  ...
  public void write(int data) {  // will also cover char
	...
	public void write(float data){
		try{
			int dataout=Float.floatToIntBits(data);
			output.write(dataout);//IDK WHY THE 0XFF IS NEEDED
			output.flush();
		}catch(Exception e){
	...
	public void write(double data){
		try{
			int dataout=(int)Double.doubleToLongBits(data);
