...
public class Riemann {
	 ...
	 * &lt; 1, the Riemann-Siegel formula is even faster. (It is not implemented
	 * yet.) For details see <a
	...
	public static double[] zeta(double[] s) {
		...
		if (Math.abs(s[0] - 1) < EPSILON && Math.abs(s[1]) < EPSILON) {
			...
			sum[0] = -.5;
			return sum;
		} else if (s[0] < 0 && Math.abs(s[0] % 2) < EPSILON
				&& Math.abs(s[1]) < EPSILON) {
			...
			if (nMax <= 0 || nMax > maxIntern) {
				System.out.println("== Accuracy +/- " + EPSILON
						+ ", requires the zeta sum to be summed up to n="
						+ nMax + ";");
				nMax = maxIntern;
			}
			...
			// choose k )!
			if (s[0] < 0 && nMax > -s[0])
				nMax = (int) Math.floor(-s[0]) + 1; // this seems to be best for
													// negative n (small
													// absolute value) ...
	 ...
	 * </tr>
	 * <tr>
	 * <td align="center"><i>t</i><sup>5</sup></td>
	 * </tr>
	 * </table>
	 * </td>
	 * </tr>
	...
	public static double Z(double t) {
		...
		double theta = t / 2 * Math.log(t / (2 * Math.PI)) - t / 2 - Math.PI
				/ 8 + 1 / (48 * t);
	 ...
	 * 
	 * double[] s = { Double.parseDouble( feld1.getText() ), Double.parseDouble(
	 * feld2.getText() ) };
	 * 
	 * double[] result = {0.,.0}; java.text.DecimalFormat digit = new
	 ...
	 * ") = "; ausgabe += Complex.toString( result, digit ); ausgabe += "   (" +
	 * digit.format(zeit - start) + " ms)";
	 * 
	 * ausgabe += "\nProbe:"; ausgabe += "\n  \u03B6(-21) = " + digit.format(
	 * -281.46 ) + " [according to Mathematica]"; ausgabe += "\n  \u03B6(-7) = "
	 * + digit.format( 1. / 240. ); ausgabe += "\n  \u03B6(-5) =" +
	 * digit.format( -1. / 252. ); ausgabe += "\n  \u03B6(-3) = " +
	 * digit.format( 1. / 120. ); ausgabe += "\n  \u03B6(-1) =" + digit.format(
	 * - 1. / 12. ); ausgabe += "\n  \u03B6(0)  = " + digit.format( - 1. / 2. );
	 * ausgabe += "\n  \u03B6(\u00BD) = " + digit.format( -1.46035 ) +
	 * " [according to Mathematica]"; ausgabe += "\n  \u03B6(2)  = " +
	 * digit.format( Math.pow(Math.PI, 2) / 6 ); ausgabe += "\n  \u03B6(3)  = "
	 * + digit.format( 1.2020569032 ); ausgabe += "\n  \u03B6(4)  = " +
	 * digit.format( Math.pow(Math.PI, 4) / 90 ); ausgabe += "\n  \u03B6(6)  = "
	 * + digit.format( Math.pow(Math.PI, 6) / 945 ); ausgabe +=
	 * "\n  \u03B6(8)  = " + digit.format( Math.pow(Math.PI, 8) / 9450 );
	 * ausgabe += "\n  \u03B6(10) = " + digit.format( Math.pow(Math.PI,10) /
	 * 93555 );
	 * 
	 * ausgabe += "\n\n  \u03C7(" + Complex.toString(s) + ") \u03C7("; ausgabe
	 * += Complex.toString( Complex.subtract(Complex.ONE_, s) ); ausgabe +=
	 * ") = "; ausgabe += Complex.toString( Complex.multiply( chi(s),
