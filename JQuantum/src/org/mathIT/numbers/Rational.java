 ...
 * Rational.java
 *
 * Copyright (C) 2007 Andreas de Vries
 *
...
/**
 * This class enables the creation of a rational number <i>n</i> &#8712; <span
 * style="font-size:large;">&#x211A;</span> and the implementation of
...
public class Rational {
	...
	public Rational(long p, long q) {
		if (p == 0 || q == 0) {
			...
			BigInteger[] cancelled = cancel(BigInteger.valueOf(p),
	...
	public Rational divide(Rational n) {
		...
		return new Rational(factor[0][0].multiply(factor[1][0]),
				factor[0][1].multiply(factor[1][1]));
	}
	...
	public Rational minus(Rational n) {
		...
		BigInteger numerator = p.multiply(denominator).divide(q)
				.subtract(n.p.multiply(denominator).divide(n.q));
	...
	public Rational plus(Rational n) {
		BigInteger denominator = lcm(this.q, n.q);
		BigInteger numerator = p.multiply(denominator).divide(q)
				.add(n.p.multiply(denominator).divide(n.q));
		return new Rational(numerator, denominator);
	}
	...
	/** For test purposes... */
	/*
	 * public static void main( String[] args ) { //Rational n = new
	 * Rational(99, 363); Rational m = new Rational( new BigInteger("990"), new
	 * BigInteger("363")); Rational n = new Rational( new BigInteger("24"), new
	 * BigInteger("-60")); System.out.println(m + " + " + n + " = " +
	 * m.plus(n)); System.out.println(m + " - " + n + " = " + m.minus(n));
	 * System.out.println(m + " * " + n + " = " + m.multiply(n));
	 * System.out.println(m + " / " + n + " = " + m.divide(n)); m = new
	 * Rational( new BigInteger("2"), new BigInteger("-13")); n = new Rational(
	 * new BigInteger("15"), new BigInteger("13")); System.out.println(m + " + "
	 * + n + " = " + m.plus(n)); System.out.println("1/"+m+" = " +
	 * m.reciprocal()); } //
	 */
}
