 ...
 *
 * Copyright (C) 2008 Andreas de Vries
 *
 ...
 * execute quantum operations on it. A quantum circuit consists of a sequence of
 * quantum gates.
 * 
 * @author Andreas de Vries
 * @version 1.2
 */
public class Circuit extends ArrayList<QuantumGate> implements
		java.io.Serializable {
	private static final long serialVersionUID = -1847355447L; // hash code of
																// "Circuit"
	/**
	 * In this array the names of all possible quantum gates are stored. These
	 * possible values are: "initialState", "Hadamard", "cNOT", "Pauli-X",
	 * "Pauli-Y", "Pauli-Z", "S", "T", "sqrt-X", "invS", "invT", "Toffoli",
	 * "QFT", "invQFT", "Function", "Rotation", "Grover", "Measurement" .
	 */
	public static String[] gatelist = { "initialState", "Hadamard", "cNOT",
			"Pauli-X", "Pauli-Y", "Pauli-Z", "S", "T", "sqrt-X", "invS",
			"invT", "Toffoli", "QFT", "invQFT", "Function", "Rotation",
			"Grover", "Measurement" };
	/** The <i>x</i>-register. */
	...
	/**
	 * The number of qubits of both the <i>x</i>-register and the
	 * <i>y</i>-register.
	 */
	private int numberOfWires = 0;
	/**
	 * Number of the gate to be executed next during the execution of this
	 * quantum circuit. If the circuit is in its initial state, the number is 0.
	 */
	 ...
	 * 
	 * @param initialQubits
	 *            the initial states of the qubits of this quantum circuit
	 */
	...
	/**
	 * Yields the current gate in this quantum circuit which is to be executed
	 * next.
	 * 
	...
	/**
	 * Returns the number of the gate to be executed next in this quantum
	 * circuit. The initial state is counted as gate number 0.
	 * 
	...
	public ArrayList<QuantumGate> getGates() {
		return new ArrayList<QuantumGate>(this);
	}

	/**
	 * Constructs and initializes this quantum cicuit according to the specified
	 * sizes of the registers and the list of quantum gates. Any previously
	 * stored quantum gates of this circuit will be deleted.
	 * 
	 * @param registerSizes
	 *            the sizes of the registers, where
	 *            <code>registerSizes[0]</code> denotes the size of the
	 *            <i>x</i>-register, and <code>registerSizes[1]</code> denotes
	 *            the size of the <i>y</i>-register
	 * @param gates
	 *            the list of quantum gates this quantum gate consists of
	 */
	 ...
	 * 
	 * @param xRegisterSize
	 *            size of the x-register
	 * @param yRegisterSize
	 *            size of the y-register
	 * @param initialState
	 *            initial state of the register
	 * @return true if and only if initialization has been successful
	 */
	public boolean initialize(int xRegisterSize, int yRegisterSize,
			int initialState) {
	...
	/**
	 * Sets the quantum cicuit into its final state. In fact, by this method the
	 * entire quantum algorithm implemented by this circuit is executed.
	 */
	 ...
	 *             if the quantum gate is unknown
	 */
	...
	/**
	 * Executes the inverse of the previously executed quantum gate in this
	 * quantum circuit.
	 * 
	 * @throws IllegalArgumentException
	 *             if the quantum gate is unknown or if it is a measurement gate
	 */
	...
	/**
	 * Determines the initial state of the quantum circuit. Here the array
	 * <code>initialQubits</code> specifies the indices of qubits either set to
	 * |0> or |1>. That is, an initial state corresponds to a classical bit
	 * state.
	 * 
	 * @param initialQubits
	 *            the indices of qubits
	 */
	...
	/**
	 * Adds a Hadamard gate on the <i>i</i>-th qubit of the quantum circuit; if
	 * the flag <code>yRegister</code> is set, it is added to the <i>i</i>-th
	 * qubit of the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of
	 * the <i>x</i>-register.
	 * 
	 * @param i
	 *            the number of qubit
	 * @param yRegister
	 *            flag whether the gate is added in the <i>y</i>-register
	 * @throws org.mathIT.quantum.NoWireException
	 *             if the register is not existing
	 */
	...
	/**
	 * Adds a CNOT gate on the two qubits specified by the two numbers in the
	 * qubit array; if the flag <code>yRegister</code> is set, it is added to
	 * the <i>y</i>-register, otherwise to the <i>x</i>-register.
	 * 
	 * @param qubits
	 *            the numbers of qubit; array size <b>must</b> be two
	 * @param yRegister
	 *            flag whether the gate is added in the <i>y</i>-register
	 * @throws org.mathIT.quantum.NoWireException
	 *             if the register is not existing
	...
	/**
	 * Adds a Pauli-X gate on the <i>i</i>-th qubit of the quantum circuit; if
	 * the flag <code>yRegister</code> is set, it is added to the <i>i</i>-th
	 * qubit of the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of
	 * the <i>x</i>-register.
	 * 
	...
	/**
	 * Adds a Pauli-Y gate on the <i>i</i>-th qubit of the quantum circuit; if
	 * the flag <code>yRegister</code> is set, it is added to the <i>i</i>-th
	 * qubit of the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of
	 * the <i>x</i>-register.
	 * 
	...
	/**
	 * Adds a Pauli-Z gate on the <i>i</i>-th qubit of the quantum circuit; if
	 * the flag <code>yRegister</code> is set, it is added to the <i>i</i>-th
	 * qubit of the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of
	 * the <i>x</i>-register.
	 * 
	...
	/**
	 * Adds an <i>S</i> gate on the <i>i</i>-th qubit of the quantum circuit; if
	 * the flag <code>yRegister</code> is set, it is added to the <i>i</i>-th
	 * qubit of the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of
	 * the <i>x</i>-register.
	 * 
	...
	/**
	 * Adds an <i>S</i><sup>*</sup> gate, the inverse or adjoint of the <i>S</i>
	 * gate, on the <i>i</i>-th qubit of the quantum circuit; if the flag
	 * <code>yRegister</code> is set, it is added to the <i>i</i>-th qubit of
	 * the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of the
	 * <i>x</i>-register.
	 * 
	 * @param i
	 *            the number of qubit
	...
	/**
	 * Adds a <i>T</i> gate on the <i>i</i>-th qubit of the quantum circuit; if
	 * the flag <code>yRegister</code> is set, it is added to the <i>i</i>-th
	 * qubit of the <i>y</i>-register, otherwise it is the <i>i</i>-th qubit of
	 * the <i>x</i>-register.
	 * 
	...
	/**
	 * Adds a &#x221A;X, or &#x221A;NOT gate on the <i>i</i>-th qubit of the
	 * quantum circuit; if the flag <code>yRegister</code> is set, it is added
	 * to the <i>i</i>-th qubit of the <i>y</i>-register, otherwise it is the
	 * <i>i</i>-th qubit of the <i>x</i>-register.
	 * 
	 * @param i
	 *            the number of qubit
	 * @param yRegister
	...
	/**
	 * Adds a Toffoli gate on the three qubits specified by the three numbers in
	 * the qubit array; if the flag <code>yRegister</code> is set, it is added
	 * to the <i>y</i>-register, otherwise to the <i>x</i>-register.
	 * 
	 * @param qubits
	 *            the numbers of qubit; array size <b>must</b> be three
	 * @param yRegister
	 *            flag whether the gate is added in the <i>y</i>-register
	 * @throws org.mathIT.quantum.NoWireException
	 *             if the register is not existing
	 */
	public void addToffoli(int[] qubits, boolean yRegister)
	...
	/**
	 * Adds the inverse Fourier transform gate to the circuit; if the flag
	 * <code>yRegister</code> is set, it is added to the <i>y</i>-register,
	 * otherwise to the <i>x</i>-register.
	 * 
	 * @param yRegister
	 *            flag whether the gate is in the <i>y</i>-register
	 * @throws org.mathIT.quantum.NoWireException
	 */
	...
	/**
	 * Adds the Fourier transform gate to the circuit; if the flag
	 * <code>yRegister</code> is set, it is added to the <i>y</i>-register,
	 * otherwise to the <i>x</i>-register.
	 * 
	 * @param yRegister
	...
	/**
	 * Adds a function evaluating gate to the circuit, specified by the input
	 * function. The <i>x</i>-register qubits save as control qubits whereas the
	 * function values are strored in the <i>y</i>-register.
	 * 
	 * @param function
	 *            the parsed function
	 * @throws org.mathIT.quantum.NoWireException
	 *             if the numbers of wires is not positive
	 */
	 ...
	 * <code>"z"</code>, respectively. The rotation angle is specified as the
	 * integer part of <i>&#x03C0;</i>.
	 * 
	 * @param qubits
	 *            the qubit numbers being involved
	 * @param yRegisterChosen
	 *            flag whether the gate is added to the <i>y</i>-register
	 * @param axis
	 *            the rotation axis
	 * @param phiAsPartOfPi
	 *            the integer part of <i>&#x03C0;</i> representing the rotation
	 *            angle
	 * @throws org.mathIT.quantum.NoWireException
	 */
	public void addRotation(int[] qubits, boolean yRegisterChosen, String axis,
			int phiAsPartOfPi) throws NoWireException {
		if (numberOfWires == 0)
			throw new NoWireException("Register not existing");
		this.add(new QuantumGate("Rotation", qubits, axis, phiAsPartOfPi,
				yRegisterChosen));
	}
	 ...
	 * 
	 * @param needle
	 *            the searched for value of the Grover gate
	 * @throws org.mathIT.quantum.NoWireException
	 */
	...
	/**
	 * Adds a measurement gate on the qubits specified by the qubit array; if
	 * the flag <code>yRegister</code> is set, it is added to the
	 * <i>y</i>-register, otherwise to the <i>x</i>-register.
	 * 
	 * @param qubits
	 *            the numbers of qubit
	 * @param yRegister
	 *            flag whether the gate is added in the <i>y</i>-register
	 * @throws org.mathIT.quantum.NoWireException
	 *             if the register is not existing
	 */
	public void addMeasurement(int[] qubits, boolean yRegister)
			throws NoWireException {
		if (numberOfWires == 0)
			throw new NoWireException("Register not existing");
	...
	/**
	 * Executes the entire quantum circuit and returns <code>true</code> if the
	...
	private void perform(QuantumGate gate) {
		if (gate.getName().equalsIgnoreCase("Hadamard")) {
			...
			if (gate.yRegister) {
				yRegister.toffoli(gate.qubits[0], gate.qubits[1],
						gate.qubits[2]);
			} else {
				xRegister.toffoli(gate.qubits[0], gate.qubits[1],
						gate.qubits[2]);
			}
			...
			if (gate.yRegister) { // ( 1 << n ) = 2^n:
				yRegister
						.inverseQft((1 << yRegisterSize), (1 << yRegisterSize));
			} else {
				xRegister
						.inverseQft((1 << xRegisterSize), (1 << xRegisterSize));
			}
			...
			if (gate.yRegister) {
				if (gate.qubits.length == 1) { // single-qubit measurement
					...
					for (int i = 0; i < yRegister.getReal().length; i++) {
						if (Math.abs(yRegister.getReal()[i]) > Register.ACCURACY
				...
				// modify y-register (works only if exclusively local gates are
				// used for y-register):
				if (xRegister.getEntanglement() != null
						&& xRegister.getEntanglement().size() > 0) {
					Integer[] x;
					for (int y : xRegister.getEntanglement().keySet()) {
						x = xRegister.getEntanglement().get(y)
					...
					HashSet<Integer> keys = new HashSet<Integer>(xRegister
							.getEntanglement().keySet());
					for (int y : keys) {
						if (xRegister.getEntanglement().get(y).size() == 0) {
			...
			throw new IllegalArgumentException("Unknown quantum gate "
					+ gate.getName());
		}
	}

	/**
	 * Modifies the x-Register in case of a y-register measurement of the
	 * specified value.
	 */
	 ...
	 * 
	 * @param gate
	 *            the quantum gate whose inverse is to execute
	 * @throws IllegalArgumentException
	 *             if the quantum gate is unknown or if it is a measurement gate
	 */
	private void unperform(QuantumGate gate) {
		if (gate.getName().equalsIgnoreCase("Hadamard")) {
			...
			throw new IllegalArgumentException(
					"Measurement gate is not reversible!");
		} else {
			throw new IllegalArgumentException("Unknown quantum gate "
					+ gate.getName());
		}
