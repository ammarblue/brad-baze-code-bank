...
public class demoSmartRockets extends PApplet {
	// Smart Rockets w/ Genetic Algorithms
	// Daniel Shiffman <http://www.shiffman.net>

	// Each Rocket's DNA is an array of PVectors
	// Each ga_Vector2 acts as a force for each frame of animation
	...
	public void draw() {
	  ...
	  text("Best cycle: " + recordtime, 10, 54);
	...
	class Obstacle {
	  ...
	  boolean contains(ga_Vector2 spot) {
	...
	class DNA {
	  ...
	  DNA() {
	    genes = new ga_Vector2[lifetime];
	    for (int i = 0; i < genes.length; i++) {
	      float angle = random(TWO_PI);
	      genes[i] = new ga_Vector2(cos(angle), sin(angle));
	      genes[i].mul(random(0, maxforce));
	    }
	  ...
	  DNA(ga_Vector2[] newgenes) {
	    // We could make a copy if necessary
	    // genes = (ga_Vector2 []) newgenes.clone();
	    genes = newgenes;
	  }
	  ...
	  DNA crossover(DNA partner) {
	    ga_Vector2[] child = new ga_Vector2[genes.length];
	    // Pick a midpoint
	  ...
	  int generations;             // Number of generations
	   ...
	   Population(float m, int num) {
	    ...
	    for (int i = 0; i < population.length; i++) {
	      ga_Vector2 location = new ga_Vector2(width/2,height+20);
	...
	class Rocket {
	  ...
	  //constructor
	  Rocket(ga_Vector2 l, DNA dna_, int totalRockets) {
	    acceleration = new ga_Vector2();
	    velocity = new ga_Vector2();
	    location = l;
	    r = 4;
	    dna = dna_;
	  ...
	  void display() {
	    float theta = (float) (Math.toRadians(velocity.angle()) + PI/2);
	    fill(200, 100);
	    stroke(0);
	    pushMatrix();
