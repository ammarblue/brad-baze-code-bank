...
public class alkanes extends PApplet {
	...
	public void setup() {
		...
		randBool(flip);
		// for (int i=1; i<=a.n; i++)
		// System.out.println(i+" "+a.endTo(i)+" "+a.startTo(i));
	}

	String names[] = { "methane", "ethane", "propane", "butane", "pentane",
	...
	public class alkane {
		...
		public alkane(int longestChain, int noSubs, int maxSubLength) {
			...
			while (true) {
				...
				for (int i = 0; i < noSubs; i++) {
					...
					subPositions[i] = ((int) random(1, maxSubLength + 0.4f))
							* 100 + j;
				}
				int k;
				if (noSubs == 0)
					...
					k = subPositions[noSubs - 1] % 100 + 1;
				for (n = k; n <= longestChain; n++)
					if (validate() == longestChain)
						return;
			}
		}
		...
		public int[] getSubs(int no) {
			...
			for (int i = 0; i < subPositions.length; i++) {
				if (subPositions[i] / 100 == no)
					x[k++] = subPositions[i] % 100;
			}
			if (k == 0)
				return null;
			int y[] = new int[k];
		...
		private int startTo(int i) {
			if (i < 1 || i > n)
				throw new RuntimeException("bounds err " + this);
