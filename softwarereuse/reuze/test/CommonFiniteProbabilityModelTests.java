...
import com.software.reuze.mp_PropositionDerivedEquivalent;
import com.software.reuze.mp_PropositionDerivedIntegerSum;
...
public abstract class CommonFiniteProbabilityModelTests extends
	...
	protected void test_RollingPairFairDiceModel_Distributions(
			mp_i_ModelFinite model) {

		mp_PropositionTermOpsAssignment ad1_1 = new mp_PropositionTermOpsAssignment(
				ExampleRV.DICE_1_RV, 1);
		mp_i_CategoricalDistributionIterator dD1_1 = model.priorDistribution(ad1_1);
		Assert.assertArrayEquals(new double[] { 1.0 / 6.0 }, dD1_1.getValues(),
				DELTA_THRESHOLD);

		mp_i_CategoricalDistributionIterator dPriorDice1 = model
		...
		mp_i_CategoricalDistributionIterator dPriorDice2 = model
				.priorDistribution(ExampleRV.DICE_2_RV);
		Assert.assertArrayEquals(new double[] { 1.0 / 6.0, 1.0 / 6.0,
				1.0 / 6.0, 1.0 / 6.0, 1.0 / 6.0, 1.0 / 6.0 },
				dPriorDice2.getValues(), DELTA_THRESHOLD);

		mp_i_CategoricalDistributionIterator dJointDice1Dice2 = model.jointDistribution(
		...
		mp_i_CategoricalDistributionIterator dJointDice2Dice1 = model.jointDistribution(
		...
		//
		// Test Sets of events
		mp_PropositionDerivedIntegerSum total11 = new mp_PropositionDerivedIntegerSum("Total",
		...
		mp_i_CategoricalDistributionIterator dPosteriorDice1GivenDice2 = model
		...
		mp_i_CategoricalDistributionIterator dPosteriorDice2GivenDice1 = model
	...
	protected void test_ToothacheCavityCatchModel_Distributions(
		...
		mp_PropositionConjunctive toothacheAndCatch = new mp_PropositionConjunctive(
				atoothache, acatch);
		Assert.assertArrayEquals(
		...
		// = P<>(Toothache | Cavity)P<>(Catch | Cavity)
		mp_PropositionConjunctive toothacheAndCatchRV = new mp_PropositionConjunctive(
	...
	protected void test_ToothacheCavityCatchWeatherModel_Distributions(
		...
		// P(sunny AND cavity)
		Assert.assertArrayEquals(new double[] { 0.12 }, model
				.priorDistribution(new mp_PropositionConjunctive(asunny, acavity))
				.getValues(), DELTA_THRESHOLD);
		// P(sunny) = <0.6>
	...
	protected void test_MeningitisStiffNeckModel_Distributions(
		...
		mp_i_CategoricalDistributionIterator dMeningitisGivenStiffNeck = model
