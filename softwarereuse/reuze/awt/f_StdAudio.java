...
public final class f_StdAudio {
    ...
    private static byte[] readByte(String filename) {
        ...
        try {
            URL url = f_StdAudio.class.getResource(filename);
    ...
    ***********************************************************************/
    ...
    private static double[] note(double hz, double duration, double amplitude) {
        int N = (int) (f_StdAudio.SAMPLE_RATE * duration);
        double[] a = new double[N+1];
        for (int i = 0; i <= N; i++)
            a[i] = amplitude * Math.sin(2 * Math.PI * i * hz / f_StdAudio.SAMPLE_RATE);
        return a;
    }
    ...
    public static void main(String[] args) {
        ...
        // 440 Hz for 1 sec
        double freq = 440.0;
        for (int i = 0; i <= f_StdAudio.SAMPLE_RATE; i++) {
            f_StdAudio.play(0.5 * Math.sin(2*Math.PI * freq * i / f_StdAudio.SAMPLE_RATE));
        }
        ...
        for (int i = 0; i < steps.length; i++) {
            ...
            f_StdAudio.play(note(hz, 1.0, 0.5));
        }
