...
public class ib_FilterFade extends ib_a_FilterPoint {
	...
	public int filterRGB(int x, int y, int rgb) {
		...
		if (sides == 2)
			...
			nx = m_MathUtils.mod(nx, 16);
		else if (sides == 4)
			nx = symmetry(nx, 16);
		int alpha = (int)(m_MathUtils.smoothStep(fadeStart, fadeStart+fadeWidth, nx) * 255);
		if (invert)
			alpha = 255-alpha;
