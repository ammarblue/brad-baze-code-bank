 ...
 * it converts the image to gray scale and then applies a color mapping based on the colors.
 */
public class ib_FilterTritone extends ib_a_FilterPoint {
    ...
    public z_BufferedImage filter( z_BufferedImage src, z_BufferedImage dst ) {
        ...
        for ( int i = 0; i < 128; i++ ) {
            float t = i / 127.0f;
            lut[i] = i_MathImageUtils.mixColors( t, shadowColor, midColor );
        }
        for ( int i = 128; i < 256; i++ ) {
            float t = (i-127) / 128.0f;
            lut[i] = i_MathImageUtils.mixColors( t, midColor, highColor );
        }
      ...
      public int filterRGB( int x, int y, int rgb ) {
        return lut[ vc_ColorOps.brightness( rgb ) ];
      }
