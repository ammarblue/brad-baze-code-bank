 ...
 */
public class ib_FilterEmboss extends ib_a_FilterWholeImage {

	private final static float pixelScale = 255.9f;

	private float azimuth = 135.0f * m_MathUtils.PI / 180.0f, elevation = 30.0f * m_MathUtils.PI / 180f;
	private boolean emboss = false;
	private float width45 = 3.0f;

	public ib_FilterEmboss() {
	}
	...
	protected int[] filterPixels( int width, int height, int[] inPixels, ga_Rectangle transformedSpace ) {
		...
		for (int i = 0; i < inPixels.length; i++)
			bumpPixels[i] = vc_ColorOps.brightness(inPixels[i]);
