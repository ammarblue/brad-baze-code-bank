 ...
 */
public class ib_FilterFieldWarp extends ib_a_FilterTransform {
		...
		if ( inLines != null && outLines != null ) {
			...
			for (int line = 0; line < inLines.length; line++) {
				Line l = intermediateLines[line] = new Line(
					m_InterpolateLerp.lerp(amount, inLines[line].x1, outLines[line].x1),
					m_InterpolateLerp.lerp(amount, inLines[line].y1, outLines[line].y1),
					m_InterpolateLerp.lerp(amount, inLines[line].x2, outLines[line].x2),
					m_InterpolateLerp.lerp(amount, inLines[line].y2, outLines[line].y2)
				);
				l.setup();
				inLines[line].setup();
			}
