...
public class ib_FilterPlasma extends ib_a_FilterWholeImage implements java.io.Serializable {

	static final long serialVersionUID = 6491871753122667752L;
	
	public float turbulence = 1.0f;
	private float scaling = 0.0f;
	private vc_i_ColorMap colormap = new vc_ColormapLinear();
	private Random randomGenerator;
	private long seed = 567;
	private boolean useColormap = false;
	private boolean useImageColors = true;

	public ib_FilterPlasma() {
	...
	private int randomRGB(int[] inPixels, int x, int y) {
		if (useImageColors) {
			return inPixels[(int) (y*originalSpace.width+x)];
		} else {
	...
	private int displace(int rgb, float amount) {
		...
		int g = (rgb >> 8) & 0xff;
		int b = rgb & 0xff;
		r = m_MathUtils.clampToByte(r + (int)(amount * (randomGenerator.nextFloat()-0.5)));
		g = m_MathUtils.clampToByte(g + (int)(amount * (randomGenerator.nextFloat()-0.5)));
		b = m_MathUtils.clampToByte(b + (int)(amount * (randomGenerator.nextFloat()-0.5)));
		return 0xff000000 | (r << 16) | (g << 8) | b;
	}

	private int average(int rgb1, int rgb2) {
		return vc_ColorOps.combinePixels(rgb1, rgb2, vc_ColorOps.AVERAGE);
	}
