 ...
 * detector.process();
 * z_BufferedImage edges = detector.getEdgesImage();
 * </code></pre>
 * 
...
public class ib_EdgeDetectionCanny {
	...
	private z_BufferedImage sourceImage;
	private z_BufferedImage edgesImage;
	...
	public z_BufferedImage getSourceImage() {
		return sourceImage;
	}
	...
	public void setSourceImage(z_BufferedImage image) {
		sourceImage = image;
	}
	 ...
	 * z_BufferedImage.TYPE_INT_ARGB in which edge pixels are white and all other
	 * pixels are black.
	 * 
	...
	public z_BufferedImage getEdgesImage() {
		return edgesImage;
	}
	...
	public void setEdgesImage(z_BufferedImage edgesImage) {
	...
	private void readLuminance() {
		...
		if (type == z_BufferedImage.TYPE_INT_RGB || type == z_BufferedImage.TYPE_INT_ARGB) {
		...
		} else if (type == z_BufferedImage.TYPE_BYTE_GRAY) {
			byte[] pixels = (byte[]) sourceImage.getData().getDataElements(0, 0, width, height, null);
			for (int i = 0; i < picsize; i++) {
				data[i] = (pixels[i] & 0xff);
			}
		} else if (type == z_BufferedImage.TYPE_USHORT_GRAY) {
			short[] pixels = (short[]) sourceImage.getData().getDataElements(0, 0, width, height, null);
			for (int i = 0; i < picsize; i++) {
				data[i] = (pixels[i] & 0xffff) / 256;
			}
		} else if (type == z_BufferedImage.TYPE_3BYTE_BGR) {
            byte[] pixels = (byte[]) sourceImage.getData().getDataElements(0, 0, width, height, null);
            int offset = 0;
            for (int i = 0; i < picsize; i++) {
	...
	private void writeEdges(int pixels[]) {
		...
		//in any other format other than an INT_ARGB type z_BufferedImage.
		//This may be easily remedied by providing alternative accessors.
		if (edgesImage == null) {
			edgesImage = new z_BufferedImage(width, height, z_BufferedImage.TYPE_INT_ARGB);
		}
