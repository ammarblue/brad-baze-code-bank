...
import com.software.reuze.m_TransformFourierReIm;
 ...
 */
public class ib_FilterLensBlur extends ib_a_Ops {
    ...
    public z_BufferedImage filter( z_BufferedImage src, z_BufferedImage dst ) {
        ...
        m_TransformFourierReIm fft = new m_TransformFourierReIm( Math.max(log2rows, log2cols) );
            ...
            for ( int x = 0; x < w; x++ ) {
				...
				if (f != 0) {
					...
					if ( sides != 0 ) {
						...
						a = m_MathUtils.mod(a, polyAngle*2)-polyAngle;
