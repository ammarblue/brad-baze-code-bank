...
public class ib_FilterShape extends ib_a_FilterWholeImage {
	...
	public ib_FilterShape() {
		colormap = new vc_ColormapLinear();
	}
	...
	private void makeMap(int[] pixels, int[] map, int width, int height) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				int offset = x + y * width;
				int b = useAlpha ? (pixels[offset] >> 24) & 0xff : vc_ColorOps.brightness(pixels[offset]);
//				map[offset] = b * one;
				map[offset] = b * one / 10;
			}
		}
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				if (m == 0) {
					...
					// get V from map
					v = m_MathUtils.clamp(factor * m / max, 0, 1);
					switch (type) {
					case CIRCLE_UP :
						v = (m_MathUtils.circleUp(v));
						break;
					case CIRCLE_DOWN :
						v = (m_MathUtils.circleDown(v));
						break;
					case SMOOTH :
					...
					sa = useAlpha ? (pixels[offset] >> 24) & 0xff : vc_ColorOps.brightness(pixels[offset]);
