...
public class ib_FilterSmear extends ib_a_FilterWholeImage implements java.io.Serializable {
	...
	public ib_FilterSmear() {
		randomGenerator = new Random();
	}
	public ib_FilterSmear(long randomSeed) {
			...
			for (i = 0; i < numShapes; i++) {
				...
				for (int x1 = x - length; x1 < x + length + 1; x1++) {
					if (x1 >= 0 && x1 < width) {
						...
						outPixels[y*width+x1] = i_MathImageUtils.mixColors(mix, rgb2, rgb);
					}
				}
				for (int y1 = y - length; y1 < y + length + 1; y1++) {
					if (y1 >= 0 && y1 < height) {
						int rgb2 = background ? 0xffffffff : outPixels[y1*width+x];
						outPixels[y1*width+x] = i_MathImageUtils.mixColors(mix, rgb2, rgb);
					}
				}
			}
			break;
		case LINES:
			...
			for (i = 0; i < numShapes; i++) {
				...
				if (x < width && x >= 0 && y < height && y >= 0) {
					...
					outPixels[y*width+x] = i_MathImageUtils.mixColors(mix, rgb2, rgb);
				}
