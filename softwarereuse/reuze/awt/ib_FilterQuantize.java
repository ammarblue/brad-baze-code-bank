...
import com.software.reuze.gb_OctTreeQuantizer;
import com.software.reuze.i_i_Quantizer;
 ...
 * is done using an oct-tree algorithm but I eventually hope to add more quantization
 ...
 */
public class ib_FilterQuantize extends ib_a_FilterWholeImage implements java.io.Serializable {
	...
	public void quantize(int[] inPixels, int[] outPixels, int width, int height, int numColors, boolean dither, boolean serpentine) {
		...
		i_i_Quantizer quantizer = new gb_OctTreeQuantizer();
							...
							for (int j = -1; j <= 1; j++) {
								...
								if (0 <= jx && jx < width) {
									...
									if (w != 0) {
										...
										g1 += eg * w/sum;
										b1 += eb * w/sum;
										inPixels[k] = (m_MathUtils.clampToByte(r1) << 16) | (m_MathUtils.clampToByte(g1) << 8) | m_MathUtils.clampToByte(b1);
									}
								}
							}
						}
