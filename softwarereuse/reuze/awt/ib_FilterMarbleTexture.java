...
public class ib_FilterMarbleTexture extends ib_a_FilterPoint implements java.io.Serializable {
...
//			f = Math.sin(f*Math.PI);
			float chaos = turbulenceFactor*m_Noise.turbulence2(nx, ny, turbulence);
//			float f = Math.sin(Math.sin(8.*chaos + 7*nx +3.*ny));
			...
			chaos = turbulenceFactor*m_Noise.turbulence2(nx, ny, turbulence);
			...
			int g = (rgb >> 8) & 0xff;
			int b = rgb & 0xff;
			r = m_MathUtils.clampToByte((int)(r*red));
			g = m_MathUtils.clampToByte((int)(g*grn));
			b = m_MathUtils.clampToByte((int)(b*blu));
