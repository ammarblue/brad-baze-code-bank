...
public class ib_FilterCrystallize extends ib_FilterCellular {
	...
	public int getPixel(int x, int y, int[] inPixels, int width, int height) {
		...
		f = m_MathUtils.smoothStep(0, edgeThickness, f);
		if (fadeEdges) {
			srcx = m_MathUtils.clamp((int)((results[1].x-1000)*scale), 0, width-1);
			srcy = m_MathUtils.clamp((int)((results[1].y-1000)*scale), 0, height-1);
			int v2 = inPixels[srcy * width + srcx];
			v2 = i_MathImageUtils.mixColors(0.5f, v2, v);
			v = i_MathImageUtils.mixColors(f, v2, v);
		} else
			v = i_MathImageUtils.mixColors(f, edgeColor, v);
		return v;
	}
