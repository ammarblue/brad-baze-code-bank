...
public class ib_FilterMaximum extends ib_a_FilterWholeImage {

	public ib_FilterMaximum() {
	}
				...
				for (int dy = -1; dy <= 1; dy++) {
					...
					if (0 <= iy && iy < height) {
						...
						for (int dx = -1; dx <= 1; dx++) {
							int ix = x+dx;
							if (0 <= ix && ix < width) {
								pixel = vc_ColorOps.combinePixels(pixel, inPixels[ioffset+ix], vc_ColorOps.MAX);
							}
						}
					}
				}
