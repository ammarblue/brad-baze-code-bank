...
public class vgu_EffectPanel extends JPanel {
	appParticleEditor editor;
	JTable emitterTable;
	DefaultTableModel emitterTableModel;
	public int editIndex;
	String lastDir;

	public vgu_EffectPanel (appParticleEditor editor) {
		this.editor = editor;
		initializeComponents();
	}

	public dg_ParticleEmitter newEmitter (String name, boolean select) {
		final dg_ParticleEmitter emitter = new dg_ParticleEmitter();

		emitter.getDuration().setLow(1000, 1000);

		emitter.getEmission().setHigh(10, 10);

		emitter.getLife().setHigh(1000, 1000);

		emitter.getScale().setHigh(4, 4);

		emitter.getRotation().setLow(1, 360);
		emitter.getRotation().setHigh(180, 180);
		emitter.getRotation().setTimeline(new float[] {0, 1});
		emitter.getRotation().setScaling(new float[] {0, 4});
		emitter.getRotation().setRelative(true);

		emitter.getAngle().setHigh(1, 360);
		emitter.getAngle().setActive(true);

		emitter.getVelocity().setHigh(120, 120);
		...
		emitter.setSprite(editor.awt);
		d_ArrayEfficientRemove<dg_ParticleEmitter> emitters = editor.effect.getEmitters();
		if (emitters.size == 0)
			emitter.setPosition(this.getWidth() / 2, this.getHeight() / 2);
		else {
			dg_ParticleEmitter p = emitters.get(0);
	...
	void openEffect () {
		...
		try {
			editor.effect.loadEmitters(new f_Handle(dir+file));
		} catch (Exception ex) {
			System.out.println("Error loading effect: " + new File(dir, file).getAbsolutePath());
			ex.printStackTrace();
			JOptionPane.showMessageDialog(editor, "Error opening effect."+dir+":"+file);
		}
		//emitterTableModel.getDataVector().removeAllElements();
		//editor.particleData.clear();
		if (editor.effect.getEmitters().size==0) return;
		for (dg_ParticleEmitter emitter : editor.effect.getEmitters()) {
			emitter.setPosition(this.getWidth() / 2, this.getHeight() / 2);
	...
	void move (int direction) {
		...
		emitterTableModel.moveRow(editIndex, editIndex, editIndex+direction);
		dg_ParticleEmitter tmp=emitters.get(editIndex);
		emitters.set(editIndex, emitters.get(editIndex+direction));
		emitters.set(editIndex+direction, tmp);
		emitterTable.getSelectionModel().setSelectionInterval(editIndex, editIndex);
	}
