 ...
 */
 public class ib_FilterBoxBlur extends ib_a_Ops {
     ...
     */
     public ib_FilterBoxBlur( float hRadius, float vRadius, int iterations ) {
     ...
     */
      ...
      public z_BufferedImage filter( z_BufferedImage src, z_BufferedImage dst ) {
        ...
        if ( premultiplyAlpha )
                  i_MathImageUtils.premultiply( inPixels, 0, inPixels.length );
        ...
        if ( premultiplyAlpha )
        	i_MathImageUtils.unpremultiply( inPixels, 0, inPixels.length );
     ...
     */
     public static void blur( int[] in, int[] out, int width, int height, float radius ) {
        ...
        for ( int y = 0; y < height; y++ ) {
            ...
            for ( int i = -r; i <= r; i++ ) {
                int rgb = in[inIndex + m_MathUtils.clamp(i, 0, width-1)];
                ta += (rgb >> 24) & 0xff;
                tr += (rgb >> 16) & 0xff;
                tg += (rgb >> 8) & 0xff;
                tb += rgb & 0xff;
