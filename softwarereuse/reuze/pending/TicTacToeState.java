 ...
 * 
 */
public class TicTacToeState implements Cloneable, Iterable<ga_XYLocation> {
	...
	public void mark(ga_XYLocation action) {
	...
	public List<ga_XYLocation> getUnMarkedPositions() {
		List<ga_XYLocation> result = new ArrayList<ga_XYLocation>();
		for (int col = 0; col < 3; col++) {
			for (int row = 0; row < 3; row++) {
				if (isEmpty(col, row)) {
					result.add(new ga_XYLocation(col, row));
				}
			}
		}
		return result;
	...
	public Iterator<ga_XYLocation> iterator() {
		// TODO Auto-generated method stub
		return getUnMarkedPositions().iterator();
	}
}
