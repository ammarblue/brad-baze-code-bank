 ...
 * abstract methods. See {@link demoRouteFindingAgent} for an example.
 * 
...
public abstract class AbstractMapAgentController extends AgentAppController {
	...
	protected com.software.reuze.das_i_SearchProblem search;
	/** Heuristic function to be used when performing informed search. */
	protected apm_a_HeuristicAdaptable heuristic;
	...
	public void update(SimulationThread simulationThread) {
		if (simulationThread.isCanceled()) {
			...
			List<aa_i_Agent> agents = scenario.getEnv().getAgents();
			if (agents.size() == 1) {
	 ...
	 */
	abstract protected apm_a_HeuristicAdaptable createHeuristic(int heuIdx);
