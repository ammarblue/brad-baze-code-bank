...
public class WorldMaze extends World {
	...
	public WorldMaze(demoGameShooterMain main){
		super(main);
	}
...
//	}
	...
	public void fillMultiPolygonsList(){
		...
		MazeCell[][] cells;
		ArrayList<ga_Polygon> wallPolygons = new ArrayList<ga_Polygon>();
		...
		for (int i = 0; i < numXAxisCells; i++){
			...
			for (int j = 0; j < numYAxisCells; j++){
				...
				if (iPlus < cells.length){
					...
					ga_Polygon verticalWallPolygon = ga_Polygon.createRectOblique(iPlus*cellWidth, j*cellHeight - wallWidth/2f, iPlus*cellWidth, jPlus*cellHeight + wallWidth/2f, wallWidth);
				...
				if (jPlus < cells[i].length){
					...
					ga_Polygon horizontalWallPolygon = ga_Polygon.createRectOblique(i*cellWidth - wallWidth/2f, jPlus*cellHeight, iPlus*cellWidth + wallWidth/2f, jPlus*cellHeight, wallWidth);
...
//		// add 4 border walls:
		wallPolygons.add(ga_Polygon.createRectOblique(0, 0, numXAxisCells*cellWidth, 0, wallWidth));
		wallPolygons.add(ga_Polygon.createRectOblique(numXAxisCells*cellWidth, 0, numXAxisCells*cellWidth, numYAxisCells*cellHeight, wallWidth));
		wallPolygons.add(ga_Polygon.createRectOblique(numXAxisCells*cellWidth, numYAxisCells*cellHeight, 0, numYAxisCells*cellHeight, wallWidth));
		wallPolygons.add(ga_Polygon.createRectOblique(0, (numYAxisCells - 1)*cellHeight, 0, 0, wallWidth));

		for (ga_Polygon polygon : wallPolygons){
			polygon.translate((float)insetXAxis, (float)insetYAxis);
		}
		...
		for (int i = 0; i < wallPolygons.size(); i++){
			ga_PolygonMultiAwt multiPolygon = new ga_PolygonMultiAwt(wallPolygons.get(i).getPolygon().copy());
	...
	public static class MazeWall {
		...
		MazeCell cell2;
		ga_Polygon polygon;
		public MazeWall(MazeCell cell, MazeCell cell2, ga_Polygon polygon){
			this.cell = cell;
			this.cell2 = cell2;
