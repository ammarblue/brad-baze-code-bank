 ...
 */
public class mp_HMMInferenceForwardBackwardConstantSpace extends mp_HMMInferenceForwardBackward {

	public mp_HMMInferenceForwardBackwardConstantSpace(mp_i_HMM hmm) {
		super(hmm);
	}
	...
	public List<mp_i_CategoricalDistributionIterator> forwardBackward(
		...
		m_Matrix f = hmm.convert(prior);
		// b, a representation of the backward message, initially all 1s
		m_Matrix b = hmm.createUnitMessage();
		// sv, a vector of smoothed estimates for steps 1,...,t
		List<m_Matrix> sv = new ArrayList<m_Matrix>(ev.size());
		...
		for (int i = ev.size() - 1; i >= 0; i--) {
			...
			m_Matrix e = hmm.getEvidence(ev.get(i));
			// b <- BACKWARD(b, ev[i])
			b = backward(b, e);
			// f1:t <-
	 ...
	 */
	public m_Matrix forwardRecover(m_Matrix O_tp1, m_Matrix f1_tp1) {
