 ...
 * 
 */
public class mpb_ParticleFiltering {

	private int N = 0;
	private mpb_i_NetworkDynamic dbn = null;
	private mp_PropositionTermOpsAssignment[][] S = new mp_PropositionTermOpsAssignment[0][0];
	//
	private m_i_Random randomizer = null;
	private mpb_PriorSample priorSampler = null;
	private mp_PropositionTermOpsAssignment[][] S_tp1 = new mp_PropositionTermOpsAssignment[0][0];
	private mp_i_ModelFinite sensorModel = null;
	private mp_i_RandomVariable sampleIndexes = null;
	 ...
	 *            <b>X</b><sub>1</sub>)
	 */
	public mpb_ParticleFiltering(int N, mpb_i_NetworkDynamic dbn) {
		this(N, dbn, new m_RandomJava());
	}
	 ...
	 */
	public mpb_ParticleFiltering(int N, mpb_i_NetworkDynamic dbn,
			m_i_Random randomizer) {
		this.randomizer = randomizer;
		this.priorSampler = new mpb_PriorSample(this.randomizer);
	 ...
	 */
	public mp_PropositionTermOpsAssignment[][] particleFiltering(mp_PropositionTermOpsAssignment[] e) {
		// local variables: W, a vector of weights of size N
		...
		for (int i = 0; i < N; i++) {
			...
			W[i] = sensorModel.posterior(mp_ProbUtil.constructConjunction(e),
					S_tp1[i]);
		}
		/* step 3 */
	 ...
	 *            <b>X</b><sub>1</sub>)
	 */
	public void initPersistent(int N, mpb_i_NetworkDynamic dbn) {
		this.N = N;
		this.dbn = dbn;
		// persistent: S, a vector of samples of size N, initially generated
		// from <b>P</b>(<b>X</b><sub>0</sub>)
		S = new mp_PropositionTermOpsAssignment[N][this.dbn.getX_0().size()];
		S_tp1 = new mp_PropositionTermOpsAssignment[N][this.dbn.getX_0().size()];
		Integer[] indexes = new Integer[N];
		for (int i = 0; i < N; i++) {
			indexes[i] = i;
			Map<mp_i_RandomVariable, Object> sample = priorSampler
					.priorSample(this.dbn.getPriorNetwork());
			int idx = 0;
			for (Map.Entry<mp_i_RandomVariable, Object> sa : sample.entrySet()) {
				S[i][idx] = new mp_PropositionTermOpsAssignment(this.dbn.getX_0_to_X_1()
						.get(sa.getKey()), sa.getValue());
				S_tp1[i][idx] = new mp_PropositionTermOpsAssignment(this.dbn
						.getX_0_to_X_1().get(sa.getKey()), sa.getValue());
				idx++;
			}
		}

		sensorModel = new mpb_ModelFinite(dbn, new mpb_InferenceEliminationAsk());

		sampleIndexes = new mp_RandomVariable("SAMPLE_INDEXES", new mp_DomainFiniteInteger(
				indexes));
	}
	...
	private void sampleFromTransitionModel(int i) {
		...
		for (int n = 0; n < S[i].length; n++) {
			mp_PropositionTermOpsAssignment x1 = S[i][n];
		...
		for (mp_i_RandomVariable X1_i : dbn.getX_1_VariablesInTopologicalOrder()) {
			// x1[i] <- a random sample from
			// <b>P</b>(X<sub>1<sub>i</sub></sub> |
			// parents(X<sub>1<sub>i</sub></sub>))
			x.put(X1_i, mp_ProbUtil.randomSample(dbn.getNode(X1_i), x, randomizer));
		}

		// S[i] <- sample from <b>P</b>(<b>X</b><sub>1</sub> |
		// <b>X</b><sub>0</sub> = S[i])
		for (int n = 0; n < S_tp1[i].length; n++) {
			mp_PropositionTermOpsAssignment x1 = S_tp1[i][n];
	 ...
	 */
	private mp_PropositionTermOpsAssignment[][] weightedSampleWithReplacement(int N,
			mp_PropositionTermOpsAssignment[][] S, double[] W) {
		mp_PropositionTermOpsAssignment[][] newS = new mp_PropositionTermOpsAssignment[N][this.dbn
				.getX_0().size()];

		double[] normalizedW = m_MathUtil2.normalize(W);

		for (int i = 0; i < N; i++) {
			int sample = (Integer) mp_ProbUtil.sample(randomizer.nextDouble(),
					sampleIndexes, normalizedW);
			for (int idx = 0; idx < S_tp1[i].length; idx++) {
				mp_PropositionTermOpsAssignment ap = S_tp1[sample][idx];
				newS[i][idx] = new mp_PropositionTermOpsAssignment(ap.getTermVariable(),
						ap.getValue());
			}
		}
