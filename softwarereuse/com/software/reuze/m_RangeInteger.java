...
public class m_RangeInteger {

    public static m_RangeInteger fromSamples(int... samples) {
        ...
        return new m_RangeInteger(min, max);
    }

    public static m_RangeInteger fromSamples(List<Integer> samples) {
        int min = Integer.MAX_VALUE;
        int max = Integer.MIN_VALUE;
        for (int s : samples) {
            min = m_MathUtils.min(min, s);
    ...
    public m_RangeInteger() {
        this(0, 100);
    }

    public m_RangeInteger(int min, int max) {
        // swap if necessary...
        if (min > max) {
            max ^= min;
            min ^= max;
    ...
    public m_RangeInteger copy() {
        m_RangeInteger range = new m_RangeInteger(min, max);
    ...
    public int pickRandom() {
        currValue = m_MathUtils.random(random, min, max);
        return currValue;
    }

    public m_RangeInteger seed(long seed) {
        random.setSeed(seed);
        return this;
    }
    ...
    public m_RangeInteger setRandom(Random rnd) {
        random = rnd;
        return this;
    }
