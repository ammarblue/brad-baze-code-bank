 ...
 * @author Ravi Mohan
 */
public class a_CellWorld<C> {
	private Set<ga_CellPositionAndContent<C>> cells = new LinkedHashSet<ga_CellPositionAndContent<C>>();
	private Map<Integer, Map<Integer, ga_CellPositionAndContent<C>>> cellLookup = new HashMap<Integer, Map<Integer, ga_CellPositionAndContent<C>>>();

	/**
	 * Construct a CellContent World with size xDimension * y Dimension cells, all with
	 ...
	 */
	public a_CellWorld(int xDimension, int yDimension, C defaultCellContent) {
		for (int x = 1; x <= xDimension; x++) {
			Map<Integer, ga_CellPositionAndContent<C>> xCol = new HashMap<Integer, ga_CellPositionAndContent<C>>();
			for (int y = 1; y <= yDimension; y++) {
				ga_CellPositionAndContent<C> c = new ga_CellPositionAndContent<C>(x, y, defaultCellContent);
				cells.add(c);
				xCol.put(y, c);
			}
	 ...
	 */
	public Set<ga_CellPositionAndContent<C>> getCells() {
		return cells;
	}
	 ...
	 * @return the CellContent an agent would end up in if they performed the specified
	 *         action from the specified cell location.
	 */
	public ga_CellPositionAndContent<C> result(ga_CellPositionAndContent<C> s, a_CellWorldAction a) {
		ga_CellPositionAndContent<C> sDelta = getCellAt(a.getXResult(s.getX()), a.getYResult(s
				.getY()));
		if (null == sDelta) {
	...
	/**
	 * Remove the cell at the specified location from this CellContent World. This
	...
	public void removeCell(int x, int y) {
		Map<Integer, ga_CellPositionAndContent<C>> xCol = cellLookup.get(x);
		if (null != xCol) {
	 ...
	 *         this location.
	 */
	public ga_CellPositionAndContent<C> getCellAt(int x, int y) {
		ga_CellPositionAndContent<C> c = null;
		Map<Integer, ga_CellPositionAndContent<C>> xCol = cellLookup.get(x);
		if (null != xCol) {
			c = xCol.get(y);
		}
