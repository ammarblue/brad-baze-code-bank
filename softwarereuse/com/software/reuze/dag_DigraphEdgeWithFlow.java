...
public class dag_DigraphEdgeWithFlow {
    private final int v;             // from
    private final int w;             // to 
    private final double capacity;   // capacity
    private double flow;             // flow

    public dag_DigraphEdgeWithFlow(int v, int w, double capacity) {
    ...
    public dag_DigraphEdgeWithFlow(int v, int w, double capacity, double flow) {
        if (capacity < 0) throw new RuntimeException("Negative edge capacity");
        this.v         = v;
        this.w         = w;  
        this.capacity  = capacity;
   ...
   /**
    ...
    public static void main(String[] args) {
        dag_DigraphEdgeWithFlow e = new dag_DigraphEdgeWithFlow(12, 23, 3.14);
