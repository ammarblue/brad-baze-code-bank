 ...
 */
public class m_NoisePerlin implements m_i_Noise {
    ...
    public float noise(float x, float y, float z) {
        if (perlin == null) {
            ...
            perlin_cosTable = m_SinCosLUT.getDefaultInstance().getSinLUT();
            perlin_TWOPI =
                    perlin_PI = m_SinCosLUT.getDefaultInstance().getPeriod();
            perlin_PI >>= 1;
        }
    ...
    private void noiseDetail(int lod) {
        if (lod > 0) {
            perlin_octaves = lod;
        }
    }

    private void noiseDetail(float falloff) {
        if (falloff > 0) {
            perlin_amp_falloff = falloff;
        }
    }

    private void noiseSeed(float what) {
        if (perlinRandom == null) {
            perlinRandom = new Random();
        }
        perlinRandom.setSeed((long)what);
        perlin = null;
    }
	...
	public void set(String name, float value) {
		if (name.charAt(0)=='s') {noiseSeed(value);}
		else if (name.charAt(0)=='l') {noiseDetail((int)value);}
		else {noiseDetail(value);}
	}

	public float getFloat(String name) {
		if (name.charAt(0)=='s') return seed;
		else if (name.charAt(0)=='l') return perlin_octaves;
		else return perlin_amp_falloff;
	}
}
