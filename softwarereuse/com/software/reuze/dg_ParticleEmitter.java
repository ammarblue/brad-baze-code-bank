...
public class dg_ParticleEmitter {
	...
	private vg_i_Sprite sprite;
	private dg_Particle[] particles;
	private int minParticleCount, maxParticleCount;
	private float x, y;
	private String name;
	private String imagePath;
	private dg_i_Particle particleType;
	...
	public dg_ParticleEmitter () {
		this.setMaxParticleCount(4); /*RPC*/
		initialize();
		particleType=dg_f_ParticleFactory.create("default");
	}

	public dg_ParticleEmitter (BufferedReader reader) throws IOException {
		initialize();
		load(reader);
		particleType=dg_f_ParticleFactory.create("default");
	}

	public dg_ParticleEmitter (dg_ParticleEmitter emitter) {
	...
	public void setMaxParticleCount (int maxParticleCount) {
		...
		activeCount = 0;
		particles = new dg_Particle[maxParticleCount];
	}
	...
	public void draw (vg_i_SpriteBatch spriteBatch) {
		if (this.particles==null || paused) return;
		if (additive) spriteBatch.setBlendFunction(vg_i_SpriteBatch.GL_SRC_ALPHA, vg_i_SpriteBatch.GL_ONE);

		dg_Particle[] particles = this.particles;
		boolean[] active = this.active;
		int activeCount = this.activeCount;
		...
		if (additive) spriteBatch.setBlendFunction(vg_i_SpriteBatch.GL_SRC_ALPHA, vg_i_SpriteBatch.GL_ONE_MINUS_SRC_ALPHA);
	}
	...
	public void draw (vg_i_SpriteBatch spriteBatch, float delta) {
		...
		for (int i = 0, n = active.length; i < n; i++) {
			if (active[i]) {
				dg_Particle particle = particles[i];
	...
	/*protected Particle newParticle (ISprite sprite) {
		return new Particle(sprite);
	}*/

	private void activateParticle (int index) {
		dg_Particle particle = particles[index];
		if (particle == null) {
			particles[index] = particle = particleType.create(sprite);
		...
		particle.lifeOffset = Math.max(100, particle.life-(int) (lifeOffset + lifeOffsetDiff * lifeOffsetValue.getScale(percent)));
		...
		if ((updateFlags & UPDATE_ANGLE) == 0) {
			...
			particle.angleCos = m_MathUtils.cosDeg(angle);
			particle.angleSin = m_MathUtils.sinDeg(angle);
		}

		float spriteWidth = (sprite==null)?1:sprite.getWidth();
		if (spriteWidth<=0) spriteWidth=1; /*RPC*/
		...
		particle.setProperty("shape="+spawnShapeValue.shape+";");
		...
		case square: {
			...
			x += m_MathUtils.random(width) - width / 2;
			y += m_MathUtils.random(height) - height / 2;
			break;
		}
		case ellipse: {
			float width = spawnWidth + (spawnWidthDiff * spawnWidthValue.getScale(percent));
			...
			if (spawnShapeValue.edges) {
				...
				case top:
					spawnAngle = -m_MathUtils.random(179f);
					break;
				case bottom:
					spawnAngle = m_MathUtils.random(179f);
					break;
				default:
					spawnAngle = m_MathUtils.random(360f);
					break;
				}
				float cosDeg = m_MathUtils.cosDeg(spawnAngle);
				float sinDeg = m_MathUtils.sinDeg(spawnAngle);
				x += cosDeg * radiusX;
				...
				while (true) {
					float px = m_MathUtils.random(width) - radiusX;
					float py = m_MathUtils.random(width) - radiusX;
		...
		case line: {
			...
			float height = spawnHeight + (spawnHeightDiff * spawnHeightValue.getScale(percent));
			if (width != 0) {
				float lineX = width * m_MathUtils.random();
				x += lineX;
				y += lineX * (height / (float)width);
			} else
				y += height * m_MathUtils.random();
			break;
		}
		case point:
		}

		float spriteHeight = (sprite==null)?1:sprite.getHeight();
		particle.setBounds(x - spriteWidth / 2, y - spriteHeight / 2, spriteWidth, spriteHeight);
	}

	private boolean updateParticle (dg_Particle particle, float delta, int deltaMillis) {
		int life = particle.currentLife - deltaMillis;
		if (life <= 0) return !particle.endOfLife();
		...
		if ((updateFlags & UPDATE_VELOCITY) != 0) {
			...
			if ((updateFlags & UPDATE_ANGLE) != 0) {
				...
				velocityX = velocity * m_MathUtils.cosDeg(angle);
				velocityY = velocity * m_MathUtils.sinDeg(angle);
				if ((updateFlags & UPDATE_ROTATION) != 0) {
	...
	public void setPosition (float x, float y) {
		...
		this.x = x;
		this.y = y;
		if (sprite != null) sprite.setBounds(x, y, spawnWidth, spawnHeight); /*rpc*/
	}
	...
	public void setSprite (vg_i_Sprite sprite) {
		...
		vg_i_Texture texture = sprite.getTexture();
	...
	public void setImagePath (String imagePath) {
		...
		sprite.create(imagePath);  //load texture
	}
	...
	static public class RangedNumericValue extends ParticleValue {
		...
		public float newLowValue () {
			return lowMin + (lowMax - lowMin) * m_MathUtils.random();
		}
	...
	static public class ScaledNumericValue extends RangedNumericValue {
		...
		public float newHighValue () {
			return highMin + (highMax - highMin) * m_MathUtils.random();
		}
