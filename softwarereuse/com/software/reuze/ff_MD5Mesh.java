...
public class ff_MD5Mesh {
	...
	public void calculateVerticesN (ff_MD5Joints skeleton, float[] verts, gb_BoundingBox bbox) {
		calculateVerticesN(skeleton, weights, vertices, verts, floatsPerVertex, floatsPerWeight, bbox);
	}

	public void calculateVertices (ff_MD5Joints skeleton, float[] verts, gb_BoundingBox bbox) {
		calculateVertices(skeleton, weights, vertices, verts, floatsPerVertex, floatsPerWeight, bbox);
	}

	static gb_Vector3 bn = new gb_Vector3();

	public static void calculateVerticesN (ff_MD5Joints skeleton, float[] weights, float vertices[], float[] verts, int vstride,
		int wstride, gb_BoundingBox bbox) {
	...
	public static void calculateVertices (ff_MD5Joints skeleton, float[] weights, float vertices[], float[] verts, int vstride,
		int wstride, gb_BoundingBox bbox) {
		for (int vertexOffset = 2, k = 0; vertexOffset < vertices.length; vertexOffset += vstride) {
			float finalX = 0;
			float finalY = 0;
	...
	public void calculateVerticesJni (ff_MD5Joints skeleton, float[] verts) {
		assert false; //MD5Jni.calculateVertices(skeleton.joints, weights, vertices, verts, numVertices);
	}

	public void calculateNormalsBind (ff_MD5Joints bindPoseSkeleton, float[] verts) {
		calculateNormalsBind(bindPoseSkeleton, weights, vertices, indices, verts, floatsPerVertex, floatsPerWeight);
	}

	static gb_Vector3 _A = new gb_Vector3();
	static gb_Vector3 _B = new gb_Vector3();
	static gb_Vector3 _n = new gb_Vector3();

	private static gb_Vector3 calcNor (gb_Vector3 v1, gb_Vector3 v2, gb_Vector3 v3) {
		_A = v2.cpy();
		_A.sub(v1);
		_B = v3.cpy();
		_B.sub(v2);
	...
	static gb_Vector3 vn = new gb_Vector3();

	public static void calculateNormalsBind (ff_MD5Joints skeleton, float[] weights, float vertices[], short indices[],
		...
		for (int i = 0; i < indices.length; i += 3) {
			...
			gb_Vector3 v1 = new gb_Vector3(verts[vo1], verts[vo1 + 1], verts[vo1 + 2]);
			gb_Vector3 v2 = new gb_Vector3(verts[vo2], verts[vo2 + 1], verts[vo2 + 2]);
			gb_Vector3 v3 = new gb_Vector3(verts[vo3], verts[vo3 + 1], verts[vo3 + 2]);

			// calculate face normal. Clockwise winding.
			gb_Vector3 fn = calcNor(v1, v2, v3);
