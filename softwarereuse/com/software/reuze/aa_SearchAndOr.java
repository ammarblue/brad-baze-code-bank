 ...
 * Actions to perform, whereas a nondeterministic search must return a aa_Plan.
 * 
 * @author Andrew Brown
 */
public class aa_SearchAndOr {
         ...
         */
        public aa_Plan search(aa_SearchProblemNondeterministic problem) {
                this.expandedNodes = 0;
                // OR-SEARCH(problem.INITIAL-STATE, problem, [])
                return this.orSearch(problem.getInitialState(), problem, new aa_Path());
        }
         ...
         * @return a conditional plan or null on failure
         */
        public aa_Plan orSearch(Object state, aa_SearchProblemNondeterministic problem,
                        aa_Path path) {
                // do metrics
                this.expandedNodes++;
                // if problem.GOAL-TEST(state) then return the empty plan
                ...
                // for each action in problem.ACTIONS(state) do
                Set<aa_i_Action> a=problem.getActionsFunction().actions(state);
                for (aa_i_Action action : a) {
                        // plan <- AND-SEARCH(RESULTS(state, action), problem, [state|path])
                        aa_Plan plan = this.andSearch(
         ...
         * @return a conditional plan or null on failure
         */
        public aa_Plan andSearch(Set<Object> states, aa_SearchProblemNondeterministic problem,
                        aa_Path path) {
                // do metrics, setup
                this.expandedNodes++;
                Object[] _states = states.toArray();
                aa_Plan[] plans = new aa_Plan[_states.length];
                ...
                if (plans.length > 0) {
                        for (int i = 0; i < plans.length - 1; i++) {
                                steps[i] = new aa_PlanIfStateThen(_states[i], plans[i]);
                        }
                        steps[steps.length-1] = plans[plans.length - 1];
                }
                
                return new aa_Plan(steps);
        }
