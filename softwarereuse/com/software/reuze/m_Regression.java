 ...
 */
public class m_Regression implements Serializable {
    ...
    public m_Regression(Vector<m_i_Number> v, Vector<m_i_Number> z) {
    	m_i_Number x,y;
    	int m=Math.min(v.size(),z.size());
    	for (int i=0; i<m; i++) {
    			x=(m_i_Number) v.elementAt(i);
    			y=(m_i_Number) z.elementAt(i);
    			addValue(x.doubleValue(),y.doubleValue());
    	}
    }   
    /**
    ...
    public String toString() {
    	double b1 = getSlope();
    	double b2 = getIntercept(b1);
    	if (b2<0) return b1+"X"+b2;
    	return b1+"X+"+b2;
    }
