...
import com.software.reuze.mp_PropositionConjunctive;
...
public class mp_ProbUtil {
	 ...
	 */
	public static int expectedSizeOfProbabilityTable(mp_i_RandomVariable... vars) {
		...
		if (null != vars) {
			for (mp_i_RandomVariable rv : vars) {
				...
				if (!(rv.getDomain() instanceof ml_i_DomainDiscreteFinite)) {
					throw new IllegalArgumentException(
							"Cannot have an infinite domain for a variable in this calculation:"
									+ rv);
				}
				ml_i_DomainDiscreteFinite d = (ml_i_DomainDiscreteFinite) rv.getDomain();
	 ...
	 *         otherwise props[0].
	 */
	public static mp_i_Proposition constructConjunction(mp_i_Proposition[] props) {
	 ...
	 */
	public static Object sample(double probabilityChoice, mp_i_RandomVariable Xi,
			double[] distribution) {
		ml_i_DomainDiscreteFinite fd = (ml_i_DomainDiscreteFinite) Xi.getDomain();
	 ...
	 *         parents(X<sub>i</sub>))
	 */
	public static Object randomSample(mpb_i_Node Xi,
			Map<mp_i_RandomVariable, Object> event, m_i_Random r) {
	 ...
	 */
	public static Object mbRandomSample(mpb_i_Node Xi,
			Map<mp_i_RandomVariable, Object> event, m_i_Random r) {
	 ...
	 * @return a random sample from <b>P</b>(X<sub>i</sub> | mb(X<sub>i</sub>))
	 */
	public static double[] mbDistribution(mpb_i_Node Xi,
			Map<mp_i_RandomVariable, Object> event) {
		ml_i_DomainDiscreteFinite fd = (ml_i_DomainDiscreteFinite) Xi.getRandomVariable().getDomain();
		...
		for (int i = 0; i < fd.size(); i++) {
			...
			// P(y<sub>j</sub>|parents(Y<sub>j</sub>))
			double cprob = 1.0;
			for (mpb_i_Node Yj : Xi.getChildren()) {
		...
		return m_MathUtil2.normalize(X);
	}
	 ...
	 *         event.
	 */
	public static Object[] getEventValuesForParents(mpb_i_Node Xi,
			Map<mp_i_RandomVariable, Object> event) {
		Object[] parentValues = new Object[Xi.getParents().size()];
		int i = 0;
		for (mpb_i_Node pn : Xi.getParents()) {
	 ...
	 *         the provided event.
	 */
	public static Object[] getEventValuesForXiGivenParents(mpb_i_Node Xi,
			Map<mp_i_RandomVariable, Object> event) {
	 ...
	 */
	public static Object[] getEventValuesForXiGivenParents(mpb_i_Node Xi,
			Object xDelta, Map<mp_i_RandomVariable, Object> event) {
	 ...
	 *         the values for X.
	 */
	public static int indexOf(mp_i_RandomVariable[] X, Map<mp_i_RandomVariable, Object> x) {
		if (0 == X.length) {
			return ((ml_i_DomainDiscreteFinite) X[0].getDomain()).getOffset(x.get(X[0]));
		}
		...
		for (int i = 0; i < X.length; i++) {
			ml_i_DomainDiscreteFinite fd = (ml_i_DomainDiscreteFinite) X[i].getDomain();
			radixValues[j] = fd.getOffset(x.get(X[i]));
			radices[j] = fd.size();
			j--;
		}

		return new m_NumberMixedRadix(radixValues, radices).intValue();
	}
	 ...
	 */
	public static int[] indexesOfValue(mp_i_RandomVariable[] X, int idx,
			Map<mp_i_RandomVariable, Object> x) {
		int csize = mp_ProbUtil.expectedSizeOfCategoricalDistribution(X);

		ml_i_DomainDiscreteFinite fd = (ml_i_DomainDiscreteFinite) X[idx].getDomain();
	...
	private static mp_i_Proposition constructConjunction(mp_i_Proposition[] props, int idx) {
		if ((idx + 1) == props.length) {
			return props[idx];
		}

		return new mp_PropositionConjunctive(props[idx], constructConjunction(
				props, idx + 1));
	}
}
