 ...
 */   
public final class ga_GeometryUtils
{
  /**
                                             ...
                                             double fractionFrom0)
  {
    ...
    double[] p = ga_GeometryUtils.computePointOnLine (p0, p1, fractionFrom0);
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength,
                                 double anchor)
  {
    double[] p = ga_GeometryUtils.computePointOnLine (p0, p1, anchor);
    
    double length0 = toLength * anchor;
    double length1 = toLength * (1.0 - anchor);
    
    ga_GeometryUtils.extendLine (p, p0, length0);
    ga_GeometryUtils.extendLine (p, p1, length1);
  }
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength)
  {
    double oldLength = ga_GeometryUtils.length (p0, p1);
  ...
  {
    double[] v = ga_GeometryUtils.createVector (p0, p1);
    return length (v);
  }
  ...
  {
    return ga_GeometryUtils.length ((double) x0, (double) y0,
                            (double) x1, (double) y1);
  }
  ...
  public static double length (int[] x, int[] y, boolean isClosed)
  {
    ...
    for (int i = 0; i < nPoints-1; i++)
      length += ga_GeometryUtils.length (x[i], y[i], x[i+1], y[i+1]);

    // Add last leg if this is a polygon
    if (isClosed && nPoints > 1)
      length += ga_GeometryUtils.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
  ...
  /**
   * Return distance between the line defined by (x0,y0) and (x1,y1)
   * and the point (x,y).
                                 ...
                                 int x, int y)
  {
    ...
    double length = ga_GeometryUtils.length (x0, y0, x1, y1);
    if (length == 0.0)
      return ga_GeometryUtils.length (x0, y0, x, y);
    ...
    length = ga_GeometryUtils.length (xp, yp, x, y);
    return length;
  }
  ...
  {
    double[] v0 = ga_GeometryUtils.createVector (p0, p1);
    double[] v1 = ga_GeometryUtils.createVector (p0, p2);

    double dotProduct = ga_GeometryUtils.computeDotProduct (v0, v1);

    double length1 = ga_GeometryUtils.length (v0);
    double length2 = ga_GeometryUtils.length (v1);
  ...
  {
    int s1 = ga_GeometryUtils.sameSide (x0, y0, x1, y1, x2, y2, x3, y3);
    int s2 = ga_GeometryUtils.sameSide (x2, y2, x3, y3, x0, y0, x1, y1);
                                                     ...
                                                     int x1, int y1)
  {
    ...
    if (ga_GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
        ga_GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
      return true;

    // If it intersects it goes through. Need to check three sides only.

    // Check against top rectangle line
    if (ga_GeometryUtils.isLineIntersectingLine (lx0, ly0, lx1, ly1,
                                         x0, y0, x1, y0))
      return true;
                                                         ...
                                                         int x1, int y1)
  {
    ...
    if (ga_GeometryUtils.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
      return true;
    ...
    for (int i = 1; i < x.length; i++) {
      if (x[i-1] != x[i] || y[i-1] != y[i])
        if (ga_GeometryUtils.isLineIntersectingRectangle (x[i-1], y[i-1],
                                                  x[i], y[i],
                                                  x0, y0, x1, y1))
          return true;
    }
                                                        ...
                                                        int x1, int y1)
  {
    ...
    if (n == 1)
      return ga_GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);
    
    //
    // If the polyline constituting the polygon intersects the rectangle
    // the polygon does too.
    //
    if (ga_GeometryUtils.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
      return true;
  
    // Check last leg as well
    if (ga_GeometryUtils.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
                                              x0, y0, x1, y1))
      return true;
    ...
    // or separate.
    //
    if (ga_GeometryUtils.isPointInsidePolygon (x, y, x0, y0) ||
        ga_GeometryUtils.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
      return true;
  ...
  /**
   * Compute centroid (center of gravity) of specified polygon.
   * 
   * @param x  X coordinates of polygon.
   * @param y  Y coordinates of polygon.   
  ...
  public static double[] computePolygonCentroid (double[] x, double[] y)
  {
    ...
    double area = ga_GeometryUtils.computePolygonArea (x, y);
                                                 ...
                                                 double[] intersection)
  {
    ...
    double a0 = ga_GeometryUtils.equals (x0, x1, LIMIT) ?
                INFINITY : (y0 - y1) / (x0 - x1);
    double a1 = ga_GeometryUtils.equals (x2, x3, LIMIT) ?
    ...
    if (ga_GeometryUtils.equals (a0, a1)) {
      if (!ga_GeometryUtils.equals (b0, b1))
        return -1; // Parallel non-overlapping
      
      else {
        if (ga_GeometryUtils.equals (x0, x1)) {
          if (Math.min (y0, y1) < Math.max (y2, y3) ||
              Math.max (y0, y1) > Math.min (y2, y3)) {
            double twoMiddle = y0 + y1 + y2 + y3 -
                               ga_GeometryUtils.min (y0, y1, y2, y3) -
                               ga_GeometryUtils.max (y0, y1, y2, y3);
            y = (twoMiddle) / 2.0;
            x = (y - b0) / a0;
          }
          else return -1;  // Parallel non-overlapping
        }
        else {
          if (Math.min (x0, x1) < Math.max (x2, x3) ||
              Math.max (x0, x1) > Math.min (x2, x3)) {
            double twoMiddle = x0 + x1 + x2 + x3 -
                               ga_GeometryUtils.min (x0, x1, x2, x3) -
                               ga_GeometryUtils.max (x0, x1, x2, x3);
            x = (twoMiddle) / 2.0;
            y = a0 * x + b0;
          }
          else return -1;
    ...
    if (ga_GeometryUtils.equals (a0, INFINITY)) {
      x = x0;
      y = a1 * x + b1;
    }
    else if (ga_GeometryUtils.equals (a1, INFINITY)) {
      x = x2;
      y = a0 * x + b0;
    }
    else {
    ...
    if (ga_GeometryUtils.equals (x0, x1)) {
      if (y0 < y1)
        distanceFrom1 = y < y0 ? ga_GeometryUtils.length (x, y, x0, y0) :
                        y > y1 ? ga_GeometryUtils.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = y < y1 ? ga_GeometryUtils.length (x, y, x1, y1) :
                        y > y0 ? ga_GeometryUtils.length (x, y, x0, y0) : 0.0;
    }
    else {
      if (x0 < x1)
        distanceFrom1 = x < x0 ? ga_GeometryUtils.length (x, y, x0, y0) :
                        x > x1 ? ga_GeometryUtils.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = x < x1 ? ga_GeometryUtils.length (x, y, x1, y1) :
                        x > x0 ? ga_GeometryUtils.length (x, y, x0, y0) : 0.0;
    }
    
    double distanceFrom2;
    if (ga_GeometryUtils.equals (x2, x3)) {
      if (y2 < y3)
        distanceFrom2 = y < y2 ? ga_GeometryUtils.length (x, y, x2, y2) :
                        y > y3 ? ga_GeometryUtils.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = y < y3 ? ga_GeometryUtils.length (x, y, x3, y3) :
                        y > y2 ? ga_GeometryUtils.length (x, y, x2, y2) : 0.0;
    }
    else {
      if (x2 < x3)
        distanceFrom2 = x < x2 ? ga_GeometryUtils.length (x, y, x2, y2) :
                        x > x3 ? ga_GeometryUtils.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = x < x3 ? ga_GeometryUtils.length (x, y, x3, y3) :
                        x > x2 ? ga_GeometryUtils.length (x, y, x2, y2) : 0.0;
    }
    
    return ga_GeometryUtils.equals (distanceFrom1, 0.0) &&
      ga_GeometryUtils.equals (distanceFrom2, 0.0) ? 1 : 0;
  }
   ...
   * is first preprocessed so that "unnecessary" vertices are removed
                                                       ...
                                                       double x1, double y1)
  {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      if (ga_GeometryUtils.equals (x2, x3) && ga_GeometryUtils.equals (y2, y3))
        continue;

      int type = ga_GeometryUtils.findLineSegmentIntersection (x0, y0, x1, y1,
                                                       x2, y2, x3, y3,
                                                       intersection);
      ...
      if (type == -2) { // Overlapping
        ...
        int side = ga_GeometryUtils.sameSide (x0, y0, x1, y1,
                                      x[p1], y[p1], x[p2], y[p2]);
  ...
  public static int[] createEllipse (int[] x, int[] y)
  {
    ...
    double axisAngle = ga_GeometryUtils.computeAngle (p0, p1, p2);
  
    // dx / dy  
    double dx = ga_GeometryUtils.length (x0, y0, x[1], y[1]);
    double dy = ga_GeometryUtils.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
    ...
    // Shear if necessary. If angle is close to 90 there is no shear.
    // If angle is close to 0 or 180 shear is infinite, and we set
    // it to zero as well.
    if (!ga_GeometryUtils.equals (axisAngle, Math.PI/2.0, 0.1) && 
        !ga_GeometryUtils.equals (axisAngle, Math.PI,     0.1) &&
        !ga_GeometryUtils.equals (axisAngle, 0.0,         0.1)) {
  ...
  public static int[] createEllipse (int x0, int y0, int dx, int dy)
  {
    ...
    // surface on a quarter of the ellipse
    ...
    // Hand code the entries in the four "corner" points of the ellipse,
    ...
    // Loop over angles from 0 to 90. The rest of the ellipse can be derived
                                        ...
                                        double dx, double dy)
  {
    ...
    // number of points in a quarter of the ellipse.
    int nPoints = 45;
                                   ...
                                   double length, double angle, double inset)
  {
    ...
    int[] pos1 = new int[2];
    ga_GeometryUtils.findPolygonPosition (x, y, length, pos1);
   ...
   * @param height  Height (may be negative to indicate upwards direction)
   */
   ...
   * @param innerRadius  Inner radius of arms.
                                             ...
                                             int[] position)
  {
    ...
    for (int i = 1; i < x.length; i++) {
      double legLength = ga_GeometryUtils.length (x[i-1], y[i-1], x[i], y[i]);
