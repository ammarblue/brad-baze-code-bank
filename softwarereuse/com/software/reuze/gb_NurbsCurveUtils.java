 ...
 * @author sg
 */
public final class gb_NurbsCurveUtils {
    ...
    public static gb_i_NurbsCurve connectCurves(gb_i_NurbsCurve curve1, gb_i_NurbsCurve curve2) {
        return connectCurves(new gb_i_NurbsCurve[] {
                curve1, curve2
        });
    }

    public static gb_i_NurbsCurve connectCurves(gb_i_NurbsCurve[] curves) {
        if (curves.length < 2) {
        ...
        LinkedList<gc_Vector4> cps = new LinkedList<gc_Vector4>();
        ...
        for (int i = 0; i < curves.length; i++) {
            ...
            gc_Vector4[] pts = curves[i].getControlPoints();
        ...
        return new gb_NurbsCurveBasic(cps.toArray(new gc_Vector4[cps.size()]), u,
                degree);
    }

    public static gb_i_NurbsCurve increaseDegree(gb_i_NurbsCurve curve, int t) {
        if (t <= 0) {
            throw new IllegalArgumentException(
                    "New degree smaller or equal degree of curve");
        }
        gc_Vector4[] cp = curve.getControlPoints();
        float[] u = curve.getKnots();
        int p = curve.getDegree();
        int seg = curve.getKnotVector().getNumberOfSegments() + 1;
        gc_Vector4[] cph = new gc_Vector4[cp.length + t * seg];
        float[] uh = new float[u.length + t * seg];

        float[][] bezalfs = new float[p + t + 1][p + 1];
        gc_Vector4[] bpts = new gc_Vector4[p + 1];
        gc_Vector4[] ebpts = new gc_Vector4[p + t + 1];
        gc_Vector4[] nextbpts = new gc_Vector4[p - 1];
        ...
        for (int i = 1; i <= ph2; i++) {
            ...
            int mpi = m_MathUtils.min(p, i);
            for (int j = m_MathUtils.max(0, i - t); j <= mpi; j++) {
                bezalfs[i][j] = inv * binp[j] * bint[i - j];
            }
        }
        for (int i = ph2 + 1; i < ph; i++) {
            double mpi = m_MathUtils.min(p, i);
            for (int j = m_MathUtils.max(0, i - t); j <= mpi; j++) {
        ...
        int cind = 1;
        float ua = u[0];
        cph[0] = new gc_Vector4(cp[0]);
        for (int i = 0; i <= ph; i++) {
            uh[i] = ua;
        }
        for (int i = 0; i <= p; i++) {
            bpts[i] = new gc_Vector4(cp[i]);
        }
        ...
        while (b < m) {
            ...
            for (int j = lbz; j <= rbz; j++) {
                cph[cind] = new gc_Vector4(ebpts[j]);
                cind++;
            }
            if (b < m) {
                for (int j = 0; j < r; j++) {
        ...
        gc_Vector4[] cpNew = new gc_Vector4[nh + 1];
        for (int i = 0; i < cpNew.length; i++) {
            cpNew[i] = new gc_Vector4(cph[i]);
        }
        return new gb_NurbsCurveBasic(cpNew, uNew, p + t);
    }

    private gb_NurbsCurveUtils() {
    }

    public gb_i_NurbsCurve equalizeConnectCurves(gb_i_NurbsCurve[] curves) {
        // TODO equalize degrees and move curves so that end of curves[i-1] is
