 ...
 * 
 */
public final class ga_Intersection {
	/**
	...
	private final static gb_Vector3 v0 = new gb_Vector3();
	private final static gb_Vector3 v1 = new gb_Vector3();
	private final static gb_Vector3 v2 = new gb_Vector3();
	 ...
	 */
	public static boolean isPointInTriangle (gb_Vector3 point, gb_Vector3 t1, gb_Vector3 t2, gb_Vector3 t3) {
		v0.set(t1).sub(point);
		v1.set(t2).sub(point);
		v2.set(t3).sub(point);
	...
	public static boolean intersectSegmentPlane (gb_Vector3 start, gb_Vector3 end, gb_Plane plane, gb_Vector3 intersection) {
		gb_Vector3 dir = end.tmp().sub(start);
	...
	/**
	 * Checks whether the given point is in the polygon. Only the x and y coordinates of the provided {@link gb_Vector3}s are used.
	 * 
	 * @param polygon The polygon vertices
	 * @param point The point
	 * @return Whether the point is in the polygon
	 */
	public static boolean isPointInPolygon (List<ga_Vector2> polygon, ga_Vector2 point) {
	 ...
	 */
	public static float distanceLinePoint (ga_Vector2 start, ga_Vector2 end, ga_Vector2 point) {
	 ...
	 */	
	public static boolean intersectSegmentCircle (ga_Vector2 start, ga_Vector2 end, ga_Vector2 center, float radius) {
		radius *= radius;
		...
		if (u <= 0) {
			...
			tmp.mul(u); //remember tmp is already normalized
			tmp2.set(tmp.x + start.x, tmp.y + start.y, 0);
		}
     ...
     * returns Vector2.ZERO on tangent
     * otherwise, intersection is set to 1st point and return vector2 is set to the 2nd point
     */
	public static ga_Vector2 intersectLineCircle (ga_Vector2 p1, ga_Vector2 p2, ga_Vector2 center, float radius, ga_Vector2 intersection/*out*/) {
		float dx=p2.x-p1.x;
		float dy=p2.y-p1.y;
		...
		if (discriminant==0) {
			intersection.set(D*dy/dr,-D*dx/dr);
			return ga_Vector2.ZERO;
		}
		discriminant=(float)Math.sqrt(discriminant);
		float dysgn=dy<0?-1:1;
		float dyabs=Math.abs(dy);
		intersection.set((D*dy+dysgn*dx*discriminant)/dr,(-D*dx+dyabs*discriminant)/dr);
		ga_Vector2 result=new ga_Vector2((D*dy-dysgn*dx*discriminant)/dr,(-D*dx-dyabs*discriminant)/dr);
		return result;
	}
	 ...
	 */
	public static float intersectSegmentCircleDisplace (ga_Vector2 start, ga_Vector2 end, ga_Vector2 point, float radius,
		ga_Vector2 displacement) {
	...
	/**
	 * Intersects a {@link gb_Ray} and a {@link gb_Plane}. The intersection point is stored in intersection in case an intersection is
	 * present.
	 * 
	 * @param ray The ray
	 ...
	 */
	public static boolean intersectRayPlane (gb_Ray ray, gb_Plane plane, gb_Vector3 intersection) {
		...
		if (denom != 0) {
			...
			return true;
		} else if (plane.testPoint(ray.origin) == gb_Plane.PlaneSide.OnPlane) {
	...
	private static final gb_Plane p = new gb_Plane(new gb_Vector3(), 0);
	private static final gb_Vector3 i = new gb_Vector3();

	/**
	 * Intersect a {@link gb_Ray} and a triangle, returning the intersection point in intersection.
	 * 
	 * @param ray The ray
	 * @param t1 The first vertex of the triangle
	 * @param t2 The second vertex of the triangle
	 ...
	 */
	public static boolean intersectRayTriangle (gb_Ray ray, gb_Vector3 t1, gb_Vector3 t2, gb_Vector3 t3, gb_Vector3 intersection) {
		p.set(t1, t2, t3);
	...
	private static final gb_Vector3 dir = new gb_Vector3();
	private static final gb_Vector3 start = new gb_Vector3();

	/**
	 * Intersects a {@link gb_Ray} and a sphere, returning the intersection point in intersection.
	 * 
	 * @param ray The ray
	 ...
	 * @return Whether an intersection is present.
	 */
	public static boolean intersectRaySphere (gb_Ray ray, gb_Vector3 center, float radius, gb_Vector3 intersection) {
	...
	/**
	 * Quick check whether the given {@link gb_Ray} and {@link gb_BoundingBox} intersect.
	 * 
	 * @param ray The ray
	 * @param box The bounding box
	 * @return Whether the ray and the bounding box intersect.
	 */
	static public boolean intersectRayBoundsFast (gb_Ray ray, gb_BoundingBox box) {
		float a, b;
		float min, max;
	...
	static gb_Vector3 best = new gb_Vector3();
	static gb_Vector3 tmp1 = new gb_Vector3();
	static gb_Vector3 tmp2 = new gb_Vector3();
	static gb_Vector3 tmp3 = new gb_Vector3();
	 ...
	 */
	public static boolean intersectRayTriangles (gb_Ray ray, float[] triangles, gb_Vector3 intersection) {
	 ...
	 * @return Whether the ray and the triangles intersect.
	 */
	public static boolean intersectRayTriangles (gb_Ray ray, float[] vertices, short[] indices, int vertexSize, gb_Vector3 intersection) {
		float min_dist = Float.MAX_VALUE;
		boolean hit = false;
	 ...
	 * @return Whether the ray and the triangles intersect.
	 */
	public static boolean intersectRayTriangles (gb_Ray ray, List<gb_Vector3> triangles, gb_Vector3 intersection) {
		float min_dist = Float.MAX_VALUE;
	 ...
	 */
	public static boolean intersectRectangles (ga_Rectangle a, ga_Rectangle b) {
	 ...
	 */
	public static boolean intersectLines (ga_Vector2 p1, ga_Vector2 p2, ga_Vector2 p3, ga_Vector2 p4, ga_Vector2 intersection) {
	 ...
	 */
	public static boolean intersectSegments (ga_Vector2 p1, ga_Vector2 p2, ga_Vector2 p3, ga_Vector2 p4, ga_Vector2 intersection) {
		float x1 = p1.x, y1 = p1.y, x2 = p2.x, y2 = p2.y, x3 = p3.x, y3 = p3.y, x4 = p4.x, y4 = p4.y;
	...
	public static boolean overlapCircles(ga_Circle c1, ga_Circle c2) {
  ...
  public static boolean overlapRectangles(ga_Rectangle r1, ga_Rectangle r2) {
  ...
  public static boolean overlapCircleRectangle(ga_Circle c, ga_Rectangle r) {
      float closestX = c.position.x;
      float closestY = c.position.y;
      ...
      closestX = closestX - c.position.x;
      closestX *= closestX;
      closestY = closestY - c.position.y;
      closestY *= closestY;
