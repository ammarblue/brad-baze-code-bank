 ...
 */
public class i_WarpGrid implements java.io.Serializable {
	...
	public i_WarpGrid(int rows, int cols, int w, int h) {
		this.rows = rows;
		this.cols = cols;
...
//			int j = row*cols;
	...
	public void lerp(float t, i_WarpGrid destination, i_WarpGrid intermediate) {
		...
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				intermediate.xGrid[index] = (float)m_InterpolateLerp.lerp(t, xGrid[index], destination.xGrid[index]);
				intermediate.yGrid[index] = (float)m_InterpolateLerp.lerp(t, yGrid[index], destination.yGrid[index]);
				index++;
			}
		}
	}
	
	public void warp(int[] inPixels, int cols, int rows, i_WarpGrid sourceGrid, i_WarpGrid destGrid, int[] outPixels) {
try {
		int x, y;
		int u, v;
		int[] intermediate;
		...
		splines = new i_WarpGrid(rows, gridCols, 1, 1);
		...
		for (y = 0; y < rows; y++) {
			...
			scale[cols] = cols;
			i_MathImageUtils.resample(inPixels, intermediate, cols, y*cols, 1, scale);
			offset += gridCols;
		}
		/* create table of y-intercepts for intermediate mesh's hor splines */

		splines = new i_WarpGrid(gridRows, cols, 1, 1);
		...
		for (x = 0; x < cols; x++) {
			...
			scale[rows] = rows;
			i_MathImageUtils.resample(intermediate, outPixels, rows, x, cols, scale);
		}
}
catch (Exception e) {
	e.printStackTrace();
