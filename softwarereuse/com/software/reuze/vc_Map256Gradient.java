	...
	private void rebuildGradient() {
		...
		for (int i = 1; i < numKnots-1; i++) {
			...
			for (int j = xKnots[i]; j < end; j++) {
				...
				z_Colors.RGBtoHSB((rgb1 >> 16) & 0xff, (rgb1 >> 8) & 0xff, rgb1 & 0xff, hsb1);
				float hsb2[]=new float[4];
				z_Colors.RGBtoHSB((rgb2 >> 16) & 0xff, (rgb2 >> 8) & 0xff, rgb2 & 0xff, hsb2);
...
//					if (blend != SPLINE) {
						...
						case HUE_CCW:
							...
							map[j] = z_Colors.HSBtoRGB((float)h, (float)s, (float)b);//FIXME-alpha
							break;
						}
//					}
				}
