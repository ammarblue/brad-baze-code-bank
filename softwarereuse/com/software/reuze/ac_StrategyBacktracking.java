 ...
 */
public class ac_StrategyBacktracking extends ac_a_SolutionStrategy {

	public ac_OpsAssignmentVariables solve(ac_CSP csp) {
		return recursiveBackTrackingSearch(csp, new ac_OpsAssignmentVariables());
	}
	 ...
	 */
	private ac_OpsAssignmentVariables recursiveBackTrackingSearch(ac_CSP csp,
			ac_OpsAssignmentVariables assignment) {
		ac_OpsAssignmentVariables result = null;
		if (assignment.isComplete(csp.getVariables())) {
			result = assignment;
		} else {
			mf_NodeTermVariable var = selectUnassignedVariable(assignment, csp);
			for (Object value : orderDomainValues(var, assignment, csp)) {
				assignment.setAssignment(var, value);
				fireStateChanged(assignment, csp);
				if (assignment.isConsistent(csp.getConstraints(var))) {
					ac_DomainRestoreInfo info = inference(var, assignment, csp);
					if (!info.isEmpty())
	 ...
	 * provided by the CSP.
	 */
	protected mf_NodeTermVariable selectUnassignedVariable(ac_OpsAssignmentVariables assignment, ac_CSP csp) {
	 ...
	 * provided by the CSP.
	 */
	protected Iterable<?> orderDomainValues(mf_NodeTermVariable var,
			ac_OpsAssignmentVariables assignment, ac_CSP csp) {
	 ...
	 */
	protected ac_DomainRestoreInfo inference(mf_NodeTermVariable var, ac_OpsAssignmentVariables assignment,
			ac_CSP csp) {
		return new ac_DomainRestoreInfo().compactify();
	}
}
