 ...
 */
public class aa_TreeSearchBestFirstSearchRecursive extends aa_TreeSearchNodeExpander implements das_i_SearchProblem {

	private final das_i_TreeSearchEvaluationFunction evaluationFunction;
	...
	public aa_TreeSearchBestFirstSearchRecursive(das_i_TreeSearchEvaluationFunction ef) {
	...
	public List<aa_i_Action> search(a_Problem p) throws Exception {
		...
		aa_TreeSearchNode n = new aa_TreeSearchNode(p.getInitialState());
		SearchResult sr = rbfs(p, n, evaluationFunction.f(n), INFINITY, 0);
		if (sr.getOutcome() == SearchResult.SearchOutcome.SOLUTION_FOUND) {
			aa_TreeSearchNode s = sr.getSolution();
			actions = aa_SearchUtils.actionsFromNodes(s.getPathFromRoot());
	...
	// a new f-cost limit
	private SearchResult rbfs(a_Problem p, aa_TreeSearchNode n, double node_f, double fLimit,
			int recursiveDepth) {

		setMaxRecursiveDepth(recursiveDepth);

		// if problem.GOAL-TEST(node.STATE) then return SOLUTION(node)
		if (aa_SearchUtils.isGoalState(p, n)) {
		...
		// for each action in problem.ACTION(node.STATE) do
		// add CHILD-NODE(problem, node, action) into successors
		List<aa_TreeSearchNode> successors = expandNode(n, p);
		// if successors is empty then return failure, infinity
...
class SearchResult {
	...
	private aa_TreeSearchNode solution;

	private SearchOutcome outcome;

	private final Double fCostLimit;

	public SearchResult(aa_TreeSearchNode solution, Double fCostLimit) {
		if (null == solution) {
	...
	public aa_TreeSearchNode getSolution() {
		return solution;
	}
