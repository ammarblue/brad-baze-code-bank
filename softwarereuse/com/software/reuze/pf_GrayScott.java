...
public class pf_GrayScott {
    ...
    public pf_GrayScott(int width, int height, boolean wrap) {
        this.width = width;
        this.height = height;
    ...
    public void seedImage(int[] pixels, int imgWidth, int imgHeight) {
        int xo = m_MathUtils.clip((width - imgWidth) / 2, 0, width - 1);
        int yo = m_MathUtils.clip((height - imgHeight) / 2, 0, height - 1);
        imgWidth = m_MathUtils.min(imgWidth, width);
        imgHeight = m_MathUtils.min(imgHeight, height);
    ...
    public void setRect(int x, int y, int w, int h) {
        int mix = m_MathUtils.clip(x - w / 2, 0, width);
        int max = m_MathUtils.clip(x + w / 2, 0, width);
        int miy = m_MathUtils.clip(y - h / 2, 0, height);
        int may = m_MathUtils.clip(y + h / 2, 0, height);
    ...
    public void setRect(ga_Rectangle r) {
        setRect((int) r.position.x, (int) r.position.y, (int) r.width, (int) r.height);
    }
