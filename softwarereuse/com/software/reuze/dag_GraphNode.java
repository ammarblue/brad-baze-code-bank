 ...
 * as duplicate ID numbers can lead to unpredictable behavior.
 * 
 * @author Peter Lager
 *
 */
public class dag_GraphNode implements Comparable<Object> {
	 ...
	 */
	public dag_GraphNode(int id, double x, double y, double z) {
		this.id = id;
		this.x = x;
		this.y = y;
		this.z = z;
	 ...
	 * @param y
	 */
	public dag_GraphNode(int id, double x, double y) {
		this.id = id;
		this.x = x;
		this.y = y;
		z = 0;
	 ...
	 */
	public dag_GraphNode(dag_GraphNode node){
		id = node.id;
		x = node.x;
		y = node.y;
		z = node.z;
	...
	public int compareTo(Object o) {
		dag_GraphNode node = (dag_GraphNode) o;
		if(id == node.id)
			return 0;
		else 
