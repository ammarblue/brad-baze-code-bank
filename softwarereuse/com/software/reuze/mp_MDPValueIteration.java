 ...
 * 
 */
public class mp_MDPValueIteration<S, A extends aa_i_Action> {
	 ...
	 *            discount &gamma; to be used.
	 */
	public mp_MDPValueIteration(double gamma) {
	 ...
	 */
	public Map<S, Double> valueIteration(mp_i_MDP<S, A> mdp,
			double epsilon) {
		//
		// local variables: U, U', vectors of utilities for states in S,
		// initially zero
		Map<S, Double> U = m_MathUtil2.create(mdp.states(), new Double(0));
		Map<S, Double> Udelta = m_MathUtil2.create(mdp.states(), new Double(0));
