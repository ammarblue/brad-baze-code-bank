...
public class gb_VoxelSpaceBrushRound extends gb_a_VoxelSpaceBrush {
     ...
     */
    public gb_VoxelSpaceBrushRound(gb_a_VoxelSpace volume, float radius) {
        super(volume);
        setSize(radius);
    }
    ...
    public void drawAtGridPos(float cx, float cy, float cz, float density) {
        int minX = m_MathUtils.max(Math.round(cx - cellRadiusX), 0);
        int minY = m_MathUtils.max(Math.round(cy - cellRadiusY), 0);
        int minZ = m_MathUtils.max(Math.round(cz - cellRadiusZ), 0);
        int maxX = m_MathUtils.min(Math.round(cx + cellRadiusX), volume.resX);
        int maxY = m_MathUtils.min(Math.round(cy + cellRadiusY), volume.resY);
        int maxZ = m_MathUtils.min(Math.round(cz + cellRadiusZ), volume.resZ);
