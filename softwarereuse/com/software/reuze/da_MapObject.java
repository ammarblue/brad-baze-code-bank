...
public class da_MapObject<K, V> {
	 ...
	 * table. */
	public da_MapObject (int initialCapacity) {
		this(initialCapacity, 0.8f);
	}

	/** Creates a new map with the specified initial capacity and load factor. This map will hold initialCapacity * loadFactor items
	 * before growing the backing table. */
	public da_MapObject (int initialCapacity, float loadFactor) {
		if (initialCapacity < 0) throw new IllegalArgumentException("initialCapacity must be >= 0: " + initialCapacity);
		if (capacity > 1 << 30) throw new IllegalArgumentException("initialCapacity is too large: " + initialCapacity);
		capacity = m_MathUtils.nextPowerOfTwo(initialCapacity);
	...
	public void putAll (da_MapObject<K, V> map) {
	...
	public void ensureCapacity (int additionalCapacity) {
		...
		if (sizeNeeded >= threshold) resize(m_MathUtils.nextPowerOfTwo((int)(sizeNeeded / loadFactor)));
	}
	...
	static private class MapIterator<K, V> {
		...
		final da_MapObject<K, V> map;
		int nextIndex, currentIndex;

		public MapIterator (da_MapObject<K, V> map) {
			this.map = map;
			reset();
		}
	...
	static public class Entries<K, V> extends MapIterator<K, V> implements Iterable<Entry<K, V>>, Iterator<Entry<K, V>> {
		...
		public Entries (da_MapObject<K, V> map) {
			super(map);
		}
	...
	static public class Values<V> extends MapIterator<Object, V> implements Iterable<V>, Iterator<V> {
		public Values (da_MapObject<?, V> map) {
			super((da_MapObject<Object, V>)map);
		}
		...
		public d_ArrayEfficientRemove<V> toArray () {
			d_ArrayEfficientRemove array = new d_ArrayEfficientRemove(true, map.size);
			while (hasNext)
				array.add(next());
			return array;
		}
		 ...
		 * @param array
		 */
		public void toArray(d_ArrayEfficientRemove<V> array) {
			while (hasNext)
				array.add(next());
		}
	}
	...
	static public class Keys<K> extends MapIterator<K, Object> implements Iterable<K>, Iterator<K> {
		public Keys (da_MapObject<K, ?> map) {
			super((da_MapObject<K, Object>)map);
		}
		...
		public d_ArrayEfficientRemove<K> toArray () {
			d_ArrayEfficientRemove array = new d_ArrayEfficientRemove(true, map.size);
			while (hasNext)
				array.add(next());
			return array;
