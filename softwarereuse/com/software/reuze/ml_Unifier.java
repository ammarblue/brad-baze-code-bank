...
public class ml_Unifier {
	...
	public Map<mf_NodeTermVariable, mf_i_NodeTerm> unify(mf_i_Node x, mf_i_Node y) {
		return unify(x, y, new LinkedHashMap<mf_NodeTermVariable, mf_i_NodeTerm>());
	}
	...
	public Map<mf_NodeTermVariable, mf_i_NodeTerm> unify(mf_i_Node x, mf_i_Node y,
		...
		if (theta == null) {
			...
			return theta;
		} else if (x instanceof mf_NodeTermVariable) {
			// else if VARIABLE?(x) then return UNIVY-VAR(x, y, theta)
			return unifyVar((mf_NodeTermVariable) x, y, theta);
		} else if (y instanceof mf_NodeTermVariable) {
			// else if VARIABLE?(y) then return UNIFY-VAR(y, x, theta)
			return unifyVar((mf_NodeTermVariable) y, x, theta);
	...
	// else if LIST?(x) and LIST?(y) then
	// return UNIFY(x.REST, y.REST, UNIFY(x.FIRST, y.FIRST, theta))
	public Map<mf_NodeTermVariable, mf_i_NodeTerm> unify(List<? extends mf_i_Node> x,
			List<? extends mf_i_Node> y, Map<mf_NodeTermVariable, mf_i_NodeTerm> theta) {
		if (theta == null) {
			return null;
	...
	protected boolean occurCheck(Map<mf_NodeTermVariable, mf_i_NodeTerm> theta, mf_NodeTermVariable var,
			mf_i_Node x) {
		// ((equal var x) t)
		if (var.equals(x)) {
			return true;
			...
			// (rest x) subst)))
		} else if (x instanceof mf_NodeTermFunction) {
			// (or (occurs-in? var (first x) subst) (occurs-in? var (rest x)
			// subst)))
			mf_NodeTermFunction fx = (mf_NodeTermFunction) x;
			for (mf_i_NodeTerm fxt : fx.getArgs()) {
	 ...
	 * </code>
	 */
	private Map<mf_NodeTermVariable, mf_i_NodeTerm> unifyVar(mf_NodeTermVariable var, mf_i_Node x,
			Map<mf_NodeTermVariable, mf_i_NodeTerm> theta) {

		if (!mf_i_NodeTerm.class.isInstance(x)) {
			return null;
			...
			// else return add {var/x} to theta
			cascadeSubstitution(theta, var, (mf_i_NodeTerm) x);
			return theta;
		}
	}

	private Map<mf_NodeTermVariable, mf_i_NodeTerm> unifyOps(String x, String y,
			Map<mf_NodeTermVariable, mf_i_NodeTerm> theta) {
		if (theta == null) {
			return null;
		} else if (x.equals(y)) {
	...
	private List<? extends mf_i_Node> args(mf_i_Node x) {
		return x.getArgs();
	}

	private String op(mf_i_Node x) {
		return x.getSymbolicName();
	}

	private boolean isCompound(mf_i_Node x) {
		return x.isCompound();
	}
	...
	// for need for this.
	private Map<mf_NodeTermVariable, mf_i_NodeTerm> cascadeSubstitution(Map<mf_NodeTermVariable, mf_i_NodeTerm> theta,
			mf_NodeTermVariable var, mf_i_NodeTerm x) {
		theta.put(var, x);
		...
		for (mf_NodeTermVariable v : theta.keySet()) {
			...
			if (t instanceof mf_NodeTermFunction) {
