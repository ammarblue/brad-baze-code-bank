...
public class da_QueuePriorityMinIndex<Key extends Comparable<Key>> implements Iterable<Integer> {
    private int N;           // number of elements on PQ
    private int[] pq;        // binary heap using 1-based indexing
    private int[] qp;        // inverse of pq - qp[pq[i]] = pq[qp[i]] = i
    private Key[] keys;      // keys[i] = priority of i

    public da_QueuePriorityMinIndex(int NMAX) {
...
*/
   ...
   /**
    ...
    private class HeapIterator implements Iterator<Integer> {
        // create a new pq
        private da_QueuePriorityMinIndex<Key> copy;

        // add all elements to copy of heap
        // takes linear time since already in heap order so no keys move
        public HeapIterator() {
            copy = new da_QueuePriorityMinIndex<Key>(pq.length - 1);
            for (int i = 1; i <= N; i++)
    ...
    public static void main(String[] args) {
        ...
        da_QueuePriorityMinIndex<String> pq = new da_QueuePriorityMinIndex<String>(strings.length);
