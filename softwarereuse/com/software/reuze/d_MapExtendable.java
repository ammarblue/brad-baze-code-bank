...
import com.software.reuze.d_GraphLabeled;
 ...
 */
public class d_MapExtendable implements d_i_Map {
	 ...
	 */
	private final d_GraphLabeled<String, Double> links;

	/** Stores xy-coordinates for each location. */
	private final Hashtable<String, ga_Point> locationPositions;

	/** Creates an empty map. */
	public d_MapExtendable() {
		links = new d_GraphLabeled<String, Double>();
		locationPositions = new Hashtable<String, ga_Point>();
	}
	...
	public String randomlyGenerateDestination() {
		return a_Utils.selectRandomlyFromList(getLocations());
	}
	...
	public void setPosition(String loc, double x, double y) {
		locationPositions.put(loc, new ga_Point(x, y));
	}
	...
	public void setDistAndDirToRefLocation(String loc, double dist, int dir) {
		ga_Point coords = new ga_Point(-Math.sin(dir * Math.PI / 180.0) * dist,
