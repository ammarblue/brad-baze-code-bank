 ...
 *  Dependencies: FlowNetwork.java DigraphEdgeFlow.java Queue.java
 *
...
public class dag_DigraphFlowNetworkFordFulkerson {
    private boolean[] marked;     // marked[v] = true iff s->v path in residual graph
    private dag_DigraphEdgeWithFlow[] edgeTo;    // edgeTo[v] = last edge on shortest residual s->v path
    private double value;         // current value of max flow
  
    // max flow in flow network G from s to t
    public dag_DigraphFlowNetworkFordFulkerson(dag_DigraphFlowNetwork G, int s, int t) {
        value = excess(G, t);
        if (!isFeasible(G, s, t)) {
    ...
    private boolean hasAugmentingPath(dag_DigraphFlowNetwork G, int s, int t) {
        edgeTo = new dag_DigraphEdgeWithFlow[G.V()];
        marked = new boolean[G.V()];

        // breadth-first search
        d_Queue<Integer> q = new d_Queue<Integer>();
        q.enqueue(s);
        marked[s] = true;
        while (!q.isEmpty()) {
            int v = q.dequeue();
    ...
    private double excess(dag_DigraphFlowNetwork G, int v) {
        double excess = 0.0;
        for (dag_DigraphEdgeWithFlow e : G.adj(v)) {
    ...
    // return excess flow at vertex v
    private boolean isFeasible(dag_DigraphFlowNetwork G, int s, int t) {
    ...
    // check optimality conditions
    private boolean check(dag_DigraphFlowNetwork G, int s, int t) {
