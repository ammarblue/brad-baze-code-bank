 ...
 * 
 */
public class ga_Vector2Quadtree extends ga_Rectangle /*implements Shape2D*/ {
    ...
    protected ga_Vector2Quadtree parent;
    protected ga_Vector2Quadtree[] children;
    ...
    /**
     * Constructs a new Quadtree node within the Rect: {o.x, o.y} ...
     * {o.x+size, o.y+size}
     * 
     * @param p
     *            parent node
     ...
     */
    private ga_Vector2Quadtree(ga_Vector2Quadtree p, ga_Vector2 o, float size) {
     ...
     */
    public ga_Vector2Quadtree(ga_Vector2 o, float size) {
        this(null, o, size);
    }
    ...
    public boolean addPoint(ga_Vector2 p) {
        ...
        if (containsPoint(p)) {
            ...
            if (halfSize <= minNodeSize) {
                ...
                if (children == null) {
                    children = new ga_Vector2Quadtree[4];
                }
                int quadrant = getQuadrantID(plocal);
                if (children[quadrant] == null) {
                    ga_Vector2 off =
                            offset.copy().add(new ga_Vector2((quadrant & 1) != 0
                                    ? halfSize
                                    : 0, (quadrant & 2) != 0 ? halfSize : 0));
                    children[quadrant] = new ga_Vector2Quadtree(this, off, halfSize);
                    numChildren++;
                }
    ...
    public boolean containsPoint(final ga_Vector2 p) {
     ...
     */
    public ga_Vector2Quadtree[] getChildren() {
        ga_Vector2Quadtree[] clones = new ga_Vector2Quadtree[4];
     ...
     */
    public ga_Vector2Quadtree getLeafForPoint(final ga_Vector2 p) {
        // if not a leaf node...
        if (this.contains(p)) {
            if (numChildren > 0) {
     ...
     * @return the parent
     */
    public ga_Vector2Quadtree getParent() {
        return parent;
    }
     ...
     */
    public ArrayList<ga_Vector2> getPointsWithinRect(ga_Rectangle r) {
     ...
     */
    public boolean remove(final ga_Vector2 p) {
        boolean found = false;
        ga_Vector2Quadtree leaf = getLeafForPoint(p);
        if (leaf != null) {
