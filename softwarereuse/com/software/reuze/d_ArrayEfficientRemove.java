 ...
 * last element is moved to the removed element's position). Even better, removes work during iteration.
 * @author Nathan Sweet */
public class d_ArrayEfficientRemove<T> implements Iterable<T> {
	/** Provides direct access to the underlying array. If the Array's generic type is not Object, this field may only be accessed
	 * if the {@link d_ArrayEfficientRemove#Array(boolean, int, Class)} constructor was used. */
	public T[] items;
	...
	public d_ArrayEfficientRemove () {
		this(true, 16);
	}

	/** Creates an ordered array with the specified capacity. */
	public d_ArrayEfficientRemove (int capacity) {
		this(true, capacity);
	}
	...
	public d_ArrayEfficientRemove (boolean ordered, int capacity) {
	 ...
	 *           memory copy.
	 * @param capacity Any elements added beyond this will cause the backing array to be grown. */
	public d_ArrayEfficientRemove (boolean ordered, int capacity, Class<T> arrayType) {
		this.ordered = ordered;
	...
	public d_ArrayEfficientRemove (Class<T> arrayType) {
	...
	public d_ArrayEfficientRemove (d_ArrayEfficientRemove array) {
	 ...
	 * grown. */
	public d_ArrayEfficientRemove (T[] array) {
		this(true, array);
	}
	 ...
	 * @param ordered If false, methods that remove elements may change the order of other elements in the array, which avoids a
	 *           memory copy. */
	public d_ArrayEfficientRemove (boolean ordered, T[] array) {
	...
	public void addAll (d_ArrayEfficientRemove array) {
		addAll(array, 0, array.size);
	}

	public void addAll (d_ArrayEfficientRemove array, int offset, int length) {
	 ...
	 * {@link Sort#instance()}). */
	public void sort () {
		Arrays.sort(items); //Sort.instance().sort(items, 0, size);
	}

	/** Sorts the array. This method is not thread safe (uses {@link Sort#instance()}). */
	public void sort (Comparator<T> comparator) {
		Arrays.sort(items); //Sort.instance().sort(items, comparator, 0, size);
	}
	...
	public T random () {
		...
		return items[m_MathUtils.random(0, size - 1)];
	}
	...
	static public class ArrayIterator<T> implements Iterator<T> {
		private final d_ArrayEfficientRemove<T> array;
		int index;

		public ArrayIterator (d_ArrayEfficientRemove<T> array) {
			this.array = array;
		}
	...
	static public class ArrayIterable<T> implements Iterable<T> {
		...
		public ArrayIterable (d_ArrayEfficientRemove<T> array) {
