...
public final class ga_Utility
{
                                             ...
                                             double fractionFrom0)
  {
    ...
    double[] p = ga_Utility.computePointOnLine (p0, p1, fractionFrom0);
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength,
                                 double anchor)
  {
    double[] p = ga_Utility.computePointOnLine (p0, p1, anchor);
    
    double length0 = toLength * anchor;
    double length1 = toLength * (1.0 - anchor);
    
    ga_Utility.extendLine (p, p0, length0);
    ga_Utility.extendLine (p, p1, length1);
  }
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength)
  {
    double oldLength = ga_Utility.length (p0, p1);
  ...
  {
    double[] v = ga_Utility.createVector (p0, p1);
    return length (v);
  }
  ...
  {
    return ga_Utility.length ((double) x0, (double) y0,
                            (double) x1, (double) y1);
  }
  ...
  public static double length (int[] x, int[] y, boolean isClosed)
  {
    ...
    for (int i = 0; i < nPoints-1; i++)
      length += ga_Utility.length (x[i], y[i], x[i+1], y[i+1]);

    // Add last leg if this is a polygon
    if (isClosed && nPoints > 1)
      length += ga_Utility.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
                                 ...
                                 int x, int y)
  {
    ...
    double length = ga_Utility.length (x0, y0, x1, y1);
    if (length == 0.0)
      return ga_Utility.length (x0, y0, x, y);
    ...
    length = ga_Utility.length (xp, yp, x, y);
    return length;
  }
  ...
  {
    double[] v0 = ga_Utility.createVector (p0, p1);
    double[] v1 = ga_Utility.createVector (p0, p2);

    double dotProduct = ga_Utility.computeDotProduct (v0, v1);

    double length1 = ga_Utility.length (v0);
    double length2 = ga_Utility.length (v1);
  ...
  {
    int s1 = ga_Utility.sameSide (x0, y0, x1, y1, x2, y2, x3, y3);
    int s2 = ga_Utility.sameSide (x2, y2, x3, y3, x0, y0, x1, y1);
                                                     ...
                                                     int x1, int y1)
  {
    ...
    if (ga_Utility.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
        ga_Utility.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
      return true;

    // If it intersects it goes through. Need to check three sides only.

    // Check against top rectangle line
    if (ga_Utility.isLineIntersectingLine (lx0, ly0, lx1, ly1,
                                         x0, y0, x1, y0))
      return true;
                                                         ...
                                                         int x1, int y1)
  {
    ...
    if (ga_Utility.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
      return true;
    ...
    for (int i = 1; i < x.length; i++) {
      if (x[i-1] != x[i] || y[i-1] != y[i])
        if (ga_Utility.isLineIntersectingRectangle (x[i-1], y[i-1],
                                                  x[i], y[i],
                                                  x0, y0, x1, y1))
          return true;
    }
                                                        ...
                                                        int x1, int y1)
  {
    ...
    if (n == 1)
      return ga_Utility.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);
    
    //
    // If the polyline constituting the polygon intersects the rectangle
    // the polygon does too.
    //
    if (ga_Utility.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
      return true;
  
    // Check last leg as well
    if (ga_Utility.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
                                              x0, y0, x1, y1))
      return true;
    ...
    // or separate.
    //
    if (ga_Utility.isPointInsidePolygon (x, y, x0, y0) ||
        ga_Utility.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
      return true;
  ...
  public static double[] computePolygonCentroid (double[] x, double[] y)
  {
    ...
    double area = ga_Utility.computePolygonArea (x, y);
                                                 ...
                                                 double[] intersection)
  {
    ...
    double a0 = ga_Utility.equals (x0, x1, LIMIT) ?
                INFINITY : (y0 - y1) / (x0 - x1);
    double a1 = ga_Utility.equals (x2, x3, LIMIT) ?
    ...
    if (ga_Utility.equals (a0, a1)) {
      if (!ga_Utility.equals (b0, b1))
        return -1; // Parallell non-overlapping
      
      else {
        if (ga_Utility.equals (x0, x1)) {
          if (Math.min (y0, y1) < Math.max (y2, y3) ||
              Math.max (y0, y1) > Math.min (y2, y3)) {
            double twoMiddle = y0 + y1 + y2 + y3 -
                               ga_Utility.min (y0, y1, y2, y3) -
                               ga_Utility.max (y0, y1, y2, y3);
            y = (twoMiddle) / 2.0;
            x = (y - b0) / a0;
          }
          ...
          if (Math.min (x0, x1) < Math.max (x2, x3) ||
            ...
            double twoMiddle = x0 + x1 + x2 + x3 -
                               ga_Utility.min (x0, x1, x2, x3) -
                               ga_Utility.max (x0, x1, x2, x3);
            x = (twoMiddle) / 2.0;
            y = a0 * x + b0;
          }
          else return -1;
    ...
    if (ga_Utility.equals (a0, INFINITY)) {
      x = x0;
      y = a1 * x + b1;
    }
    else if (ga_Utility.equals (a1, INFINITY)) {
      x = x2;
      y = a0 * x + b0;
    }
    else {
    ...
    if (ga_Utility.equals (x0, x1)) {
      if (y0 < y1)
        distanceFrom1 = y < y0 ? ga_Utility.length (x, y, x0, y0) :
                        y > y1 ? ga_Utility.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = y < y1 ? ga_Utility.length (x, y, x1, y1) :
                        y > y0 ? ga_Utility.length (x, y, x0, y0) : 0.0;
    }
    else {
      if (x0 < x1)
        distanceFrom1 = x < x0 ? ga_Utility.length (x, y, x0, y0) :
                        x > x1 ? ga_Utility.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = x < x1 ? ga_Utility.length (x, y, x1, y1) :
                        x > x0 ? ga_Utility.length (x, y, x0, y0) : 0.0;
    }
    
    double distanceFrom2;
    if (ga_Utility.equals (x2, x3)) {
      if (y2 < y3)
        distanceFrom2 = y < y2 ? ga_Utility.length (x, y, x2, y2) :
                        y > y3 ? ga_Utility.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = y < y3 ? ga_Utility.length (x, y, x3, y3) :
                        y > y2 ? ga_Utility.length (x, y, x2, y2) : 0.0;
    }
    else {
      if (x2 < x3)
        distanceFrom2 = x < x2 ? ga_Utility.length (x, y, x2, y2) :
                        x > x3 ? ga_Utility.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = x < x3 ? ga_Utility.length (x, y, x3, y3) :
                        x > x2 ? ga_Utility.length (x, y, x2, y2) : 0.0;
    }
    
    return ga_Utility.equals (distanceFrom1, 0.0) &&
      ga_Utility.equals (distanceFrom2, 0.0) ? 1 : 0;
  }
                                                       ...
                                                       double x1, double y1)
  {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      if (ga_Utility.equals (x2, x3) && ga_Utility.equals (y2, y3))
        continue;

      int type = ga_Utility.findLineSegmentIntersection (x0, y0, x1, y1,
                                                       x2, y2, x3, y3,
                                                       intersection);
      ...
      if (type == -2) { // Overlapping
        ...
        int side = ga_Utility.sameSide (x0, y0, x1, y1,
                                      x[p1], y[p1], x[p2], y[p2]);
  ...
  public static int[] createEllipse (int[] x, int[] y)
  {
    ...
    double axisAngle = ga_Utility.computeAngle (p0, p1, p2);
  
    // dx / dy  
    double dx = ga_Utility.length (x0, y0, x[1], y[1]);
    double dy = ga_Utility.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
    ...
    // it to zero as well.
    if (!ga_Utility.equals (axisAngle, Math.PI/2.0, 0.1) && 
        !ga_Utility.equals (axisAngle, Math.PI,     0.1) &&
        !ga_Utility.equals (axisAngle, 0.0,         0.1)) {
                                   ...
                                   double length, double angle, double inset)
  {
    ...
    int[] pos1 = new int[2];
    ga_Utility.findPolygonPosition (x, y, length, pos1);
                                             ...
                                             int[] position)
  {
    ...
    for (int i = 1; i < x.length; i++) {
      double legLength = ga_Utility.length (x[i-1], y[i-1], x[i], y[i]);
