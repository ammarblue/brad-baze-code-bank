...
public class gb_SplineCatmull implements Serializable {
	private static final long serialVersionUID = -3290464799289771451L;
	private List<gb_Vector3> controlPoints = new ArrayList<gb_Vector3>();
	...
	public void add (gb_Vector3 point) {
		controlPoints.add(point);
	}

	/** @return all control points */
	public List<gb_Vector3> getControlPoints () {
	 ...
	 * The first and the last control point are omitted. if there's less than 4 control points an empty path is returned.
	 * 
	 * @param numPoints number of points returned for a segment
	 * @return the path */
	public List<gb_Vector3> getPath (int numPoints) {
		ArrayList<gb_Vector3> points = new ArrayList<gb_Vector3>();
		...
		for (int i = 1; i <= controlPoints.size() - 3; i++) {
			...
			for (int j = 0; j < numPoints; j++) {
				...
				gb_Vector3 point = new gb_Vector3(controlPoints.get(i)).mul(h1);
	 ...
	 * The first and the last control point are omitted. if there's less than 4 control points an empty path is returned.
	 * 
	 * @param points the array of gb_Vector3 instances to store the path in
	 * @param numPoints number of points returned for a segment */
	public void getPath (gb_Vector3[] points, int numPoints) {
		int idx = 0;
		...
		for (int i = 1; i <= controlPoints.size() - 3; i++) {
			...
			for (int j = 0; j < numPoints; j++) {
				...
				gb_Vector3 point = points[idx++].set(controlPoints.get(i)).mul(h1);
				point.add(controlPoints.get(i + 1).tmp().mul(h2));
				point.add(T1.tmp().mul(h3));
				point.add(T2.tmp().mul(h4));
				t += increment;
	...
	public List<gb_Vector3> getTangents (int numPoints) {
		ArrayList<gb_Vector3> tangents = new ArrayList<gb_Vector3>();
		...
		for (int i = 1; i <= controlPoints.size() - 3; i++) {
			...
			tangents.add(new gb_Vector3(T1).nor());
	...
	/** Returns all tangent's normals in 2D space for the points in a path. The control points have to lie in the x/y plane for this
	 * to work. Same semantics as getPath.
	 * 
	 * @param numPoints number of points returned for a segment
	 * @return the tangents of the points in the path */
	public List<gb_Vector3> getTangentNormals2D (int numPoints) {
		ArrayList<gb_Vector3> tangents = new ArrayList<gb_Vector3>();
		...
		for (int i = 1; i <= controlPoints.size() - 3; i++) {
			...
			gb_Vector3 normal = new gb_Vector3(T1).nor();
			float x = normal.x;
			normal.x = normal.y;
			normal.y = -x;
			tangents.add(normal);
	...
	public List<gb_Vector3> getTangentNormals (int numPoints, gb_Vector3 up) {
		List<gb_Vector3> tangents = getTangents(numPoints);
		ArrayList<gb_Vector3> normals = new ArrayList<gb_Vector3>();

		for (gb_Vector3 tangent : tangents)
			normals.add(new gb_Vector3(tangent).crs(up).nor());

		return normals;
	}

	public List<gb_Vector3> getTangentNormals (int numPoints, List<gb_Vector3> up) {
		List<gb_Vector3> tangents = getTangents(numPoints);
		ArrayList<gb_Vector3> normals = new ArrayList<gb_Vector3>();

		int i = 0;
		for (gb_Vector3 tangent : tangents)
			normals.add(new gb_Vector3(tangent).crs(up.get(i++)).nor());
