 ...
 * 
 */
public class gb_PointOctree extends gb_AABB3 {
	 ...
	 * 
	 */
    protected gb_PointOctree parent;

    protected gb_PointOctree[] children;
     ...
     */
    private gb_PointOctree(gb_PointOctree p, gb_Vector3 o, float halfSize) {
     ...
     */
    public gb_PointOctree(gb_Vector3 o, float size) {
        this(null, o, size / 2);
    }
    ...
    public boolean addPoint(gb_Vector3 p) {
        ...
        if (contains(p)) {
            ...
            if (halfSize <= minNodeSize) {
                ...
                if (children == null) {
                    children = new gb_PointOctree[8];
                }
                int octant = getOctantID(plocal);
                if (children[octant] == null) {
                    gb_Vector3 off = offset.cpy().add(new gb_Vector3(
                            (octant & 1) != 0 ? halfSize : 0,
                            (octant & 2) != 0 ? halfSize : 0,
                            (octant & 4) != 0 ? halfSize : 0));
                    children[octant] = new gb_PointOctree(this, off,
                            halfSize * 0.5f);
                    numChildren++;
                }
     ...
     * all of its children.
     */
    public void applyVisitor(gb_i_OctreeVisitor visitor) {
        visitor.visitNode(this);
        if (numChildren > 0) {
            for (gb_PointOctree c : children) {
                if (c != null) {
     ...
     */
    public gb_PointOctree[] getChildren() {
        if (children != null) {
            gb_PointOctree[] clones = new gb_PointOctree[8];
     ...
     */
    public gb_PointOctree getLeafForPoint(gb_Vector3 p) {
        // if not a leaf node...
        if (this.contains(p)) {
            if (numChildren > 0) {
                int octant = getOctantID(p.tmp().sub(offset));
                if (children[octant] != null) {
     ...
     * @return octant index
     */
    protected final int getOctantID(gb_Vector3 plocal) {
     ...
     * @return the parent
     */
    public gb_PointOctree getParent() {
        return parent;
    }
     ...
     * @param b
     *            AABB
     * @return all points within the box volume
     */
    public List<gb_Vector3> getPointsWithinBox(gb_AABB3 b) {
        ...
        if (this.intersectsBox(b)) {
            if (points != null) {
                ...
                for (int i = 0; i < 8; i++) {
                    if (children[i] != null) {
                        List<gb_Vector3> points = children[i]
     ...
     */
    public List<gb_Vector3> getPointsWithinSphere(gb_Sphere s) {
        ArrayList<gb_Vector3> results = null;
     ...
     * @return selected points
     */
    public List<gb_Vector3> getPointsWithinSphere(gb_Vector3 sphereOrigin,
            float clipRadius) {
     ...
     */
    public boolean remove(gb_Vector3 p) {
        boolean found = false;
        gb_PointOctree leaf = getLeafForPoint(p);
        if (leaf != null) {
