...
/** Loads an {@link ff_MD5Model} MD5 (Doom 3) model.
...
public class ff_MD5Loader {

	public static ff_MD5Model loadModel (InputStream in, boolean allocateNormals) {
		BufferedReader reader = new BufferedReader(new InputStreamReader(in), 1024);
		ff_MD5Model model = new ff_MD5Model();
		List<String> tokens = new ArrayList<String>(10);
		gb_Quaternion quat = new gb_Quaternion();
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (tokens.get(0).equals("numJoints")) {
					...
					model.baseSkeleton = new ff_MD5Joints();
				...
				if (tokens.get(0).equals("numMeshes")) {
					...
					model.meshes = new ff_MD5Mesh[numMeshes];
				}
				...
				if (tokens.get(0).equals("mesh") && tokens.get(1).equals("{")) {
					ff_MD5Mesh mesh = new ff_MD5Mesh();
	...
	public static ff_MD5Animation loadAnimation (InputStream in) {
		BufferedReader reader = new BufferedReader(new InputStreamReader(in));
		List<String> tokens = new ArrayList<String>();
		ff_MD5Animation animation = new ff_MD5Animation();
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (tokens.get(0).equals("numFrames")) {
					...
					animation.frames = new ff_MD5Joints[numFrames];
					animation.bounds = new gb_BoundingBox[numFrames];
				}

				if (tokens.get(0).equals("numJoints")) {
					int numJoints = parseInt(tokens.get(1));
					for (int i = 0; i < animation.frames.length; i++) {
						animation.frames[i] = new ff_MD5Joints();
				...
				if (tokens.get(0).equals("bounds")) {
					for (int i = 0; i < animation.bounds.length; i++) {
						...
						gb_BoundingBox bounds = new gb_BoundingBox();
	...
	static gb_Quaternion thisOrient = new gb_Quaternion();
	static gb_Quaternion parentOrient = new gb_Quaternion();
	static gb_Vector3 parentPos = new gb_Vector3();
	...
	private static void buildFrameSkeleton (JointInfo[] jointInfos, BaseFrameJoint[] baseFrame, float[] animFrameData,
		ff_MD5Animation animation, int frameIndex) {
		ff_MD5Joints skelFrame = animation.frames[frameIndex];

		for (int i = 0; i < jointInfos.length; i++) {
			BaseFrameJoint baseJoint = baseFrame[i];
			gb_Vector3 animatedPos = new gb_Vector3();
			gb_Quaternion animatedOrient = new gb_Quaternion();
			int j = 0;
			...
			if (jointInfos[i].parent < 0) {
				...
				parentOrient.rotateVector(animatedPos);
				...
				parentOrient.mul(animatedOrient);
				parentOrient.nor();
	...
	static class BaseFrameJoint {
		public final gb_Vector3 pos = new gb_Vector3();
		public final gb_Quaternion orient = new gb_Quaternion();
	}
	public static void main(String args[]) {
		ff_MD5Model model=null;
		try {
			model = ff_MD5Loader.loadModel(new FileInputStream("data/zfat.md5mesh"), true);
