...
public class m_Matrix2D {
	 ...
	 */
	public List<ga_Vector2D> transformVector2D(final List<ga_Vector2D> vList){
		List<ga_Vector2D> transformed = new LinkedList<ga_Vector2D>();
		Iterator<ga_Vector2D> iter = vList.iterator();
		while(iter.hasNext()){
			ga_Vector2D v = iter.next();
			double x = (matrix._11 * v.x) + (matrix._21 * v.y) + (matrix._31);
			double y = (matrix._12 * v.x) + (matrix._22 * v.y) + (matrix._32);
			transformed.add(new ga_Vector2D(x,y));
		}
		return transformed;
	}

	/**
	 * Create a vector from the provided vector after being transformed by the
	 * matrix.
	 * 
	 * @param vPoint the original vector
	 * @return the transformed vector
	 */
	public ga_Vector2D transformVector2D(final ga_Vector2D vPoint){
	...
	public void rotate(final double rot){
		Matrix mat = new Matrix();
		double sinA = m_MathFast.sin(rot);
		double cosA = m_MathFast.cos(rot);
	 ...
	 * @param side
	 */
	public void rotate(final ga_Vector2D fwd, final ga_Vector2D side){
		Matrix mat = new Matrix();
