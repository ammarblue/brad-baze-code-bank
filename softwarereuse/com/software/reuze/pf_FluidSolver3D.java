...
public class pf_FluidSolver3D {
    ...
    public pf_FluidSolver3D(int size, float diffusion, float viscosity, float dt) {
        this.size = size;
    ...
    public void addVelocity(int x, int y, int z, gb_Vector3 amount) {
        int idx = IX(x, y, z);
        velX[idx] += amount.x;
        velY[idx] += amount.y;
        velZ[idx] += amount.z;
    ...
    public void setAt(int x, int y, int z, float dens, gb_Vector3 vel) {
        final int idx = IX(x, y, z);
        if (idx >= 0 && idx < density.length) {
            density[idx] = dens;
            velX[idx] = vel.x;
