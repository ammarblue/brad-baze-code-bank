 ...
 * 
 */
public class gb_BoundingBox implements Serializable {
	private static final long serialVersionUID = -1286036817192127343L;
	final gb_Vector3 crn[] = new gb_Vector3[8];
	public final gb_Vector3 min = new gb_Vector3();
	public final gb_Vector3 max = new gb_Vector3();
	final gb_Vector3 cnt = new gb_Vector3();
	final gb_Vector3 dim = new gb_Vector3();
	 ...
	 */
	public gb_Vector3 getDimensions () {
		return dim;
	}
	 ...
	 */
	public synchronized gb_Vector3 getMax () {
		return max;
	}
	 ...
	 */
	public gb_BoundingBox () {
		crn_dirty = true;
		for (int l_idx = 0; l_idx < 8; l_idx++)
			crn[l_idx] = new gb_Vector3();
		clr();
	}
	 ...
	 */
	public gb_BoundingBox (gb_BoundingBox bounds) {
		crn_dirty = true;
		for (int l_idx = 0; l_idx < 8; l_idx++)
			crn[l_idx] = new gb_Vector3();
		this.set(bounds);
	 ...
	 */
	public gb_BoundingBox (gb_Vector3 minimum, gb_Vector3 maximum) {
		crn_dirty = true;
		for (int l_idx = 0; l_idx < 8; l_idx++)
			crn[l_idx] = new gb_Vector3();
	 ...
	 */
	public gb_BoundingBox set (gb_BoundingBox bounds) {
		crn_dirty = true;
	 ...
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox set (gb_Vector3 minimum, gb_Vector3 maximum) {
	 ...
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox set (gb_Vector3[] points) {
		this.inf();
		for (gb_Vector3 l_point : points)
			this.ext(l_point);
		crn_dirty = true;
		return this;
	}
	 ...
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox set (List<gb_Vector3> points) {
		this.inf();
		for (gb_Vector3 l_point : points)
			this.ext(l_point);
		crn_dirty = true;
	...
	/**
	 * Extends the bounding box to incorporate the given {@link gb_Vector3}.
	 * 
	 * @param point The vector
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox ext (gb_Vector3 point) {
		crn_dirty = true;
	 ...
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox ext (gb_BoundingBox a_bounds) {
		crn_dirty = true;
	 ...
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox mul (m_Matrix4 matrix) {
		updateCorners();
		this.inf();
		for (gb_Vector3 l_pnt : crn) {
			l_pnt.mul(matrix);
	 ...
	 */
	public boolean contains (gb_BoundingBox bounds) {
	 ...
	 */
	public boolean contains (gb_Vector3 v) {
	 ...
	 * @return This bounding box for chaining.
	 */
	public gb_BoundingBox ext (float x, float y, float z) {
		crn_dirty = true;
