...
public class ff_MD5Animation extends va_a_Animation {
	public int frameRate;
	public float secondsPerFrame;

	public ff_MD5Joints[] frames;
	public gb_BoundingBox[] bounds;
	public String name;
	...
	public void read (DataInputStream in) throws IOException {
		...
		frames = new ff_MD5Joints[numFrames];
		for (int i = 0; i < numFrames; i++) {
			frames[i] = new ff_MD5Joints();
			frames[i].read(in);
		}
		int numBounds = in.readInt();
		bounds = new gb_BoundingBox[numBounds];
		for (int i = 0; i < numBounds; i++) {
			bounds[i] = new gb_BoundingBox();
	...
	static gb_Quaternion jointAOrient = new gb_Quaternion();
	static gb_Quaternion jointBOrient = new gb_Quaternion();

	public static void interpolate (ff_MD5Joints skeletonA, ff_MD5Joints skeletonB, ff_MD5Joints skeletonOut, float t) {
	...
	public static void main(String args[]) {
		ff_MD5Animation anim=null;
		try {
			anim = ff_MD5Loader.loadAnimation(new FileInputStream("data/walk1.md5anim"));
