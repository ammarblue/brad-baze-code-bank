 ...
 */
public class m_MatrixDouble implements java.io.Serializable, Cloneable {
    ...
    private static void chase_across(double[] s, double[] e, int k, m_MatrixDouble u) {
        double f, g, r;
        double[] cosl = new double[1];
        double[] sinl = new double[1];
        int i;
        m_MatrixDouble t = new m_MatrixDouble(u.nRow, u.nCol);
        m_MatrixDouble m = new m_MatrixDouble(u.nRow, u.nCol);
    ...
    private static void chase_up(double[] s, double[] e, int k, m_MatrixDouble v) {
        double f, g, r;
        double[] cosr = new double[1];
        double[] sinr = new double[1];
        int i;
        m_MatrixDouble t = new m_MatrixDouble(v.nRow, v.nCol);
        m_MatrixDouble m = new m_MatrixDouble(v.nRow, v.nCol);
    ...
    private static void checkMatrix(m_MatrixDouble m) {
        int i, j;

        for (i = 0; i < m.nRow; i++) {
            for (j = 0; j < m.nCol; j++) {
                if (m_MathUtils.abs(m.values[i][j]) < 0.0000000001) {
    ...
    private static int compute_2X2(double f, double g, double h,
        ...
        ft = f;
        fa = m_MathUtils.abs(ft);
        ht = h;
        ha = m_MathUtils.abs(h);
        ...
        if (ga == 0.0) {
            ...
            if (gasmal) {
                ...
                if (gasmal) {
                    ...
                    if (mm == 0.0) {
                        if (l == 0.0) {
                            t = m_MathUtils.dualSign(c_b3, ft)
                                    * m_MathUtils.dualSign(c_b4, gt);
                        } else {
                            t = gt / m_MathUtils.dualSign(d, ft) + m / t;
                        }
                    } else {
                        t = (m / (s + t) + m / (r + l)) * (a + 1.0);
                    }
            ...
            if (pmax == 1) {
                tsign = m_MathUtils.dualSign(c_b4, csr[0])
                        * m_MathUtils.dualSign(c_b4, csl[0])
                        * m_MathUtils.dualSign(c_b4, f);
            }
            if (pmax == 2) {
                tsign = m_MathUtils.dualSign(c_b4, snr[0])
                        * m_MathUtils.dualSign(c_b4, csl[0])
                        * m_MathUtils.dualSign(c_b4, g);
            }
            if (pmax == 3) {
                tsign = m_MathUtils.dualSign(c_b4, snr[0])
                        * m_MathUtils.dualSign(c_b4, snl[0])
                        * m_MathUtils.dualSign(c_b4, h);
            }

            single_values[index] = m_MathUtils.dualSign(ssmax, tsign);
            d__1 = tsign * m_MathUtils.dualSign(c_b4, f)
                    * m_MathUtils.dualSign(c_b4, h);
            single_values[index + 1] = m_MathUtils.dualSign(ssmin, d__1);
        }
    ...
    private static double compute_rot(double f, double g, double[] sin,
        ...
        if (g == 0.0) {
            ...
            f1 = f;
            g1 = g;
            scale = m_MathUtils.max(m_MathUtils.abs(f1), m_MathUtils.abs(g1));
            if (scale >= safmx2) {
                count = 0;
                while (scale >= safmx2) {
                    ++count;
            ...
            if (m_MathUtils.abs(f) > m_MathUtils.abs(g) && cs < 0.0) {
                cs = -cs;
                sn = -sn;
                r = -r;
            }
    ...
    private static double compute_shift(double f, double g, double h) {
        ...
        fa = m_MathUtils.abs(f);
        ga = m_MathUtils.abs(g);
        ha = m_MathUtils.abs(h);
        fhmn = m_MathUtils.min(fa, ha);
        fhmx = m_MathUtils.max(fa, ha);

        if (fhmn == 0.0) {
            ssmin = 0.0;
            if (fhmx == 0.0) {
            } else {
                d__1 = m_MathUtils.min(fhmx, ga) / m_MathUtils.max(fhmx, ga);
            }
        } else {
            if (ga < fhmx) {
                as = fhmn / fhmx + 1.0;
    ...
    public static void computeQR(int start, int end, double[] s, double[] e,
            m_MatrixDouble u, m_MatrixDouble v) {
        ...
        for (k = 0; k < MAX_INTERATIONS && !converged; k++) {
            for (i = start; i <= end; i++) {
                ...
                if (i == start) {
                    ...
                    f = (m_MathUtils.abs(s[i]) - shift)
                            * (m_MathUtils.dualSign(1.0, s[i]) + shift / s[i]);
                    g = e[i];
                }
            ...
            while ((end - start > 1) && (m_MathUtils.abs(e[end]) < CONVERGE_TOL)) {
                end--;
            }

            // check if need to split
            for (n = end - 2; n > start; n--) {
                if (m_MathUtils.abs(e[n]) < CONVERGE_TOL) { // split
                    computeQR(n + 1, end, s, e, u, v); // do lower matrix
                    end = n - 1; // do upper matrix

                    // check for convergence on off diagonals and reduce
                    while ((end - start > 1)
                            && (m_MathUtils.abs(e[end]) < CONVERGE_TOL)) {
                        end--;
                    }
                }
            }
            ...
            if ((end - start <= 1)
                    && (m_MathUtils.abs(e[start + 1]) < CONVERGE_TOL)) {
                converged = true;
            } else {
        ...
        if (m_MathUtils.abs(e[1]) < CONVERGE_TOL) {
    ...
    public static int computeSVD(m_MatrixDouble mat, m_MatrixDouble U, m_MatrixDouble W, m_MatrixDouble V) {
        int i, j, k;
        int nr, nc, si;
        ...
        m_MatrixDouble tmp = new m_MatrixDouble(mat.nRow, mat.nCol);
        m_MatrixDouble u = new m_MatrixDouble(mat.nRow, mat.nCol);
        m_MatrixDouble v = new m_MatrixDouble(mat.nRow, mat.nCol);
        m_MatrixDouble m = new m_MatrixDouble(mat);
            ...
            if (nr > 1) {
                ...
                if (m.values[si][si] == 0.0) {
                    ...
                    vec[0] = m.values[si][si]
                            + m_MathUtils.dualSign(mag, m.values[si][si]);
                }
            ...
            if (nc > 2) {
                ...
                if (m.values[si][si + 1] == 0.0) {
                    ...
                    vec[0] = m.values[si][si + 1]
                            + m_MathUtils.dualSign(mag, m.values[si][si + 1]);
                }
    ...
    public static boolean decomposeLU(int dim, double[] matrix0,
        ...
        for (j = 0; j < dim; j++) {
            ...
            for (i = j; i < dim; i++) {
                ...
                if ((temp = row_scale[i] * m_MathUtils.abs(sum)) >= big) {
                    big = temp;
                    imax = i;
                }
            }
    ...
    private static void print_m(m_MatrixDouble m, m_MatrixDouble u, m_MatrixDouble v) {
        m_MatrixDouble mtmp = new m_MatrixDouble(m.nCol, m.nRow);
    ...
    private static void print_svd(double[] s, double[] e, m_MatrixDouble u, m_MatrixDouble v) {
        int i;
        m_MatrixDouble mtmp = new m_MatrixDouble(u.nCol, v.nRow);
    ...
    private static String toString(m_MatrixDouble m) {
        StringBuffer buffer = new StringBuffer(m.nRow * m.nCol * 8);
        int i, j;

        for (i = 0; i < m.nRow; i++) {
            for (j = 0; j < m.nCol; j++) {
                if (m_MathUtils.abs(m.values[i][j]) < m_MathUtils.EPS) {
    ...
    private static void update_u(int index, m_MatrixDouble u, double[] cosl,
            double[] sinl) {
        int j;
        double utemp;
    ...
    private static void update_u_split(int topr, int bottomr, m_MatrixDouble u,
            double[] cosl, double[] sinl, m_MatrixDouble t, m_MatrixDouble m) {
        int j;
        double utemp;
    ...
    private static void update_v(int index, m_MatrixDouble v, double[] cosr,
            double[] sinr) {
        int j;
        double vtemp;
    ...
    private static void update_v_split(int topr, int bottomr, m_MatrixDouble v,
            double[] cosr, double[] sinr, m_MatrixDouble t, m_MatrixDouble m) {
        int j;
        double vtemp;
     ...
     */
    public m_MatrixDouble(m_MatrixDouble matrix) {
        nRow = matrix.nRow;
        nCol = matrix.nCol;
     ...
     */
    public m_MatrixDouble(int nRow, int nCol) {
        values = new double[nRow][nCol];
        this.nRow = nRow;
        this.nCol = nCol;
     ...
     */
    public m_MatrixDouble(int nRow, int nCol, double[] matrix) {
        values = new double[nRow][nCol];
        this.nRow = nRow;
        this.nCol = nCol;
     ...
     *            the other matrix
     */
    public final void add(m_MatrixDouble m1) {
        int i, j;
     ...
     *            the second matrix
     */
    public final void add(m_MatrixDouble m1, m_MatrixDouble m2) {
        int i, j;
     ...
     */
    public Object clone() {
        m_MatrixDouble m1 = null;
        try {
            m1 = (m_MatrixDouble) super.clone();
     ...
     *         or odd respectively
     */
    public final int computeLUD(m_MatrixDouble LU, d_VectorDouble permutation) {
        ...
        if (!decomposeLU(LU.nRow, temp, row_perm, even_row_exchange)) {
            ...
            throw new le_ExceptionSingularMatrix();
        }
     ...
     */
    public final int computeSVD(m_MatrixDouble U, m_MatrixDouble W, m_MatrixDouble V) {
    ...
    public final void copySubMatrix(int rowSource, int colSource, int numRow,
            int numCol, int rowDest, int colDest, m_MatrixDouble target) {
        int i, j;
     ...
     *            the threshold value
     */
    public boolean epsilonEquals(m_MatrixDouble m1, double epsilon) {
        int i, j;
        double diff;
     ...
     */
    @Deprecated
    public boolean epsilonEquals(m_MatrixDouble m1, float epsilon) {
     ...
     */
    public boolean equals(m_MatrixDouble m1) {
        try {
            int i, j;
    ...
    public boolean equals(Object o1) {
        try {
            m_MatrixDouble m2 = (m_MatrixDouble) o1;
            int i, j;
     ...
     */
    public final void get(m_MatrixDouble m1) {
        int i, j, nc, nr;
     ...
     *            The matrix that will hold the new values
     */
    public final void get(m_Matrix3 m1) {
        if (nRow < 3 || nCol < 3) {
            m1.set(0);
            if (nCol > 0) {
                if (nRow > 0) {
                    m1.vals[0] = (float) values[0][0];
                    if (nRow > 1) {
                        m1.vals[3] = (float) values[1][0];
                        if (nRow > 2) {
                            m1.vals[6] = (float) values[2][0];
                        }
                    }
                }
                if (nCol > 1) {
                    if (nRow > 0) {
                        m1.vals[1] = (float) values[0][1];
                        if (nRow > 1) {
                            m1.vals[4] = (float) values[1][1];
                            if (nRow > 2) {
                                m1.vals[7] = (float) values[2][1];
                            }
                        }
                    }
                    if (nCol > 2) {
                        if (nRow > 0) {
                            m1.vals[2] = (float) values[0][2];
                            if (nRow > 1) {
                                m1.vals[5] = (float) values[1][2];
                                if (nRow > 2) {
                                    m1.vals[8] = (float) values[2][2];
                                }
                            }
                        }
                    }
     ...
     *            The matrix that will hold the new values
     */
    public final void get(m_Matrix4 m1) {

        if (nRow < 4 || nCol < 4) {
            m1.set(0f);
            if (nCol > 0) {
                if (nRow > 0) {
                    m1.val[m_Matrix4.M00] = (float) values[0][0];
                    if (nRow > 1) {
                        m1.val[m_Matrix4.M10] = (float) values[1][0];
                        if (nRow > 2) {
                            m1.val[m_Matrix4.M20] = (float) values[2][0];
                            if (nRow > 3) {
                                m1.val[m_Matrix4.M30] = (float) values[3][0];
                            }
                        }
                    }
                }
                if (nCol > 1) {
                    if (nRow > 0) {
                        m1.val[m_Matrix4.M01] = (float) values[0][1];
                        if (nRow > 1) {
                            m1.val[m_Matrix4.M11] = (float) values[1][1];
                            if (nRow > 2) {
                                m1.val[m_Matrix4.M21] = (float) values[2][1];
                                if (nRow > 3) {
                                    m1.val[m_Matrix4.M31] = (float) values[3][1];
                                }
                            }
                        }
                    }
                    if (nCol > 2) {
                        if (nRow > 0) {
                            m1.val[m_Matrix4.M02] = (float) values[0][2];
                            if (nRow > 1) {
                                m1.val[m_Matrix4.M12] = (float) values[1][2];
                                if (nRow > 2) {
                                    m1.val[m_Matrix4.M22] = (float) values[2][2];
                                    if (nRow > 3) {
                                        m1.val[m_Matrix4.M32] = (float) values[3][2];
                                    }
                                }
                            }
                        }
                        if (nCol > 3) {
                            if (nRow > 0) {
                                m1.val[m_Matrix4.M03] = (float) values[0][3];
                                if (nRow > 1) {
                                    m1.val[m_Matrix4.M13] = (float) values[1][3];
                                    if (nRow > 2) {
                                        m1.val[m_Matrix4.M23] = (float) values[2][3];
                                        if (nRow > 3) {
                                            m1.val[m_Matrix4.M33] = (float) values[3][3];
                                        }
                                    }
                                }
                            }
     ...
     */
    public final void getColumn(int col, d_VectorDouble vector) {
        if (vector.size() < nRow) {
            vector.setSize(nRow);
        }
     ...
     */
    public final void getRow(int row, d_VectorDouble vector) {
        if (vector.size() < nCol) {
            vector.setSize(nCol);
        }
     ...
     */
    public final void invert(m_MatrixDouble m1) {
        invertGeneral(m1);
    }
     ...
     */
    final void invertGeneral(m_MatrixDouble m1) {
     ...
     *            the other matrix
     */
    public final void mul(m_MatrixDouble m1) {
        int i, j, k;
     ...
     *            the second matrix
     */
    public final void mul(m_MatrixDouble m1, m_MatrixDouble m2) {
        int i, j, k;
     ...
     */
    public final void mul(d_VectorDouble v1, d_VectorDouble v2) {
        int i, j;
     ...
     */
    public final void mulTransposeBoth(m_MatrixDouble m1, m_MatrixDouble m2) {
        int i, j, k;
     ...
     *            The matrix on the right hand side of the multiplication
     */
    public final void mulTransposeLeft(m_MatrixDouble m1, m_MatrixDouble m2) {
        int i, j, k;
     ...
     *            The matrix on the right hand side of the multiplication
     */
    public final void mulTransposeRight(m_MatrixDouble m1, m_MatrixDouble m2) {
        int i, j, k;
     ...
     *            The source matrix
     */
    public final void negate(m_MatrixDouble m1) {
        int i, j;
        if (nRow != m1.nRow || nCol != m1.nCol) {
            throw new le_ExceptionMatrixSize();
        }
     ...
     *            the source matrix
     */
    public final void set(m_MatrixDouble m1) {
        int i, j;
     ...
     *            the matrix
     */
    public final void set(m_Matrix3 m1) {
        if (nRow < 3 || nCol < 3) {
        ...
        values[0][0] = m1.vals[0];
        values[0][1] = m1.vals[1];
        values[0][2] = m1.vals[2];

        values[1][0] = m1.vals[3];
        values[1][1] = m1.vals[4];
        values[1][2] = m1.vals[5];

        values[2][0] = m1.vals[6];
        values[2][1] = m1.vals[7];
        values[2][2] = m1.vals[8];
     ...
     *            the matrix
     */
    public final void set(m_Matrix4 m1) {
        if (nRow < 4 || nCol < 4) {
        ...
        values[0][0] = m1.val[m_Matrix4.M00];
        values[0][1] = m1.val[m_Matrix4.M01];
        values[0][2] = m1.val[m_Matrix4.M02];
        values[0][3] = m1.val[m_Matrix4.M03];

        values[1][0] = m1.val[m_Matrix4.M10];
        values[1][1] = m1.val[m_Matrix4.M11];
        values[1][2] = m1.val[m_Matrix4.M12];
        values[1][3] = m1.val[m_Matrix4.M13];

        values[2][0] = m1.val[m_Matrix4.M20];
        values[2][1] = m1.val[m_Matrix4.M21];
        values[2][2] = m1.val[m_Matrix4.M22];
        values[2][3] = m1.val[m_Matrix4.M23];

        values[3][0] = m1.val[m_Matrix4.M30];
        values[3][1] = m1.val[m_Matrix4.M31];
        values[3][2] = m1.val[m_Matrix4.M32];
        values[3][3] = m1.val[m_Matrix4.M33];
     ...
     *            the source vector
     */
    public final void setColumn(int col, d_VectorDouble vector) {
        for (int i = 0; i < nRow; i++) {
     ...
     *            the source vector
     */
    public final void setRow(int row, d_VectorDouble vector) {
        for (int i = 0; i < nCol; i++) {
     ...
     *            the other matrix
     */
    public final void sub(m_MatrixDouble m1) {
        int i, j;
        if (nRow != m1.nRow) {
            throw new le_ExceptionMatrixSize();
        }
     ...
     *            the second matrix
     */
    public final void sub(m_MatrixDouble m1, m_MatrixDouble m2) {
        int i, j;
        if (m2.nRow != m1.nRow) {
            throw new le_ExceptionMatrixSize();
        }
     ...
     */
    public final void transpose(m_MatrixDouble m1) {
        int i, j;
