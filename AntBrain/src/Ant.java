...
public class Ant extends PApplet {
	...
	Ant(NeuralNet net) {
		...
		fitness = 0;
		position = new Coordinate(random(SIMULATION_WIDTH),
	...
	int checkForMine(Coordinate[] mines) {
		Coordinate DistToObject = new Coordinate(position.x
				- mines[closestMinePosition].x, position.y
				- mines[closestMinePosition].y);
	...
	Coordinate GetClosestMine(Coordinate[] mines) {
		...
		for (int i = 0; i < mines.length; i++) {
			double lengthToObject = VectorLength(new Coordinate(mines[i].x
					- position.x, mines[i].y - position.y));

			if (lengthToObject < closestSoFar) {
				closestSoFar = lengthToObject;

				closestObject = new Coordinate(position.x - mines[i].x,
						position.y - mines[i].y);
	...
	void update(Coordinate[] mines) {
		...
		position.setLocation(position.x + (direction.x * speed), position.y
				+ (direction.y * speed));
