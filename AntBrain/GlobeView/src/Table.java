...
public class Table extends PApplet{
	...
	Table(String nombre) {
		...
		for (int i = 0; i < filas.length; i++) {
			...
			if (filas[i].startsWith("#")) { // startsWith() doesn't work on
											// processingjs
				continue;
			}
	...
	// Returns value as String | be careful with method overloading using
	// processingjs
	// String getString(String rowName, int col) { return
	// getString(getRowIndex(rowName),col); }
	String getString(int rowIndex, int colIndex) {
		return data[rowIndex][colIndex];
	}

	// Returns value as Int | be careful, bla, bla..
	// int getInt(String rowName, int col) { return
	// parseInt(getString(rowName,col));}
	int getInt(int rowIndex, int colIndex) {
		return parseInt(getString(rowIndex, colIndex));
	}

	// Returns value as Float | be careful, bla, bla..
	// float getFloat(String rowName, int col) { return
	// parseFloat(getString(rowName,col)); }
