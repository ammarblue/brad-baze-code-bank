...
public class WorldVis extends PApplet {
	int X, Y;
	static final int TEXT_COL = 0xaa000000, DATA_COL = 0x99ff0000,
			HOVER_COL = 0xffff0000, WORLD_TINT = 0xffffffff, LINES_WEIGHT = 2,
			BUFF_LINES_W = 6;
	float a, b;
	PGraphics bg, hover;
	PFont h0, h1, h2, h3;
	...
	public void setup() {
		// Buffers
		size(900, 700, P3D);
		bg = createGraphics(width, height, P2D); // Buffer for storing the
													// background
	...
	void createBackground(PGraphics pg, int X, int Y, float f) {
		...
		for (int i = 0; ++i < pg.width * pg.height;) {
			pg.set(x = i % pg.width, y = i / pg.width,
					(255 - round(dist(x, y, X, Y) * f)) * 0x010101);
		}
		pg.endDraw();
		background(pg);
	}
