...
public class GraphWorkshop extends PApplet {
	...
	class Graph {
		...
		Vertex addVertex(PVector pVector) {
			...
			if (vertexSet.size() < MAX_VERTICES) {
				...
				vertex = new Vertex(vertexLabel, color(
	...
	class Edge {
		...
		Edge(Vertex startVertex, Vertex endVertex, int edgeWeight,
				boolean directional) {
		...
		void show(boolean showEdgeLabel) {
			...
			if (toggleDigraph) {
				float lineAngle = atan2(startLocation.y - endLocation.y,
						startLocation.x - endLocation.x);
				float arrowAngle = PI / 12;
				float arrowSize = 9;
				fill(EDGE_COLOR);
				triangle(lerp(startLocation.x, endLocation.x, 0.5f),
						lerp(startLocation.y, endLocation.y, 0.5f),
						lerp(startLocation.x, endLocation.x, 0.5f)
								+ (arrowSize * cos(lineAngle + arrowAngle)),
						lerp(startLocation.y, endLocation.y, 0.5f)
								+ (arrowSize * sin(lineAngle + arrowAngle)),
						lerp(startLocation.x, endLocation.x, 0.5f)
								+ (arrowSize * cos(lineAngle - arrowAngle)),
						lerp(startLocation.y, endLocation.y, 0.5f)
								+ (arrowSize * sin(lineAngle - arrowAngle)));
			...
			if (showEdgeLabel) {
				fill(EDGE_COLOR);
				text(edgeWeight, lerp(startLocation.x, endLocation.x, 0.5f) + 5,
						lerp(startLocation.y, endLocation.y, 0.5f) - 5);
			}
		}
	...
	class Vertex implements Comparable<Vertex> {
		...
		Vertex(String vertexLabel, int vertexColor, PVector vertexLocation) {
		...
		void show(boolean showVertexLabel) {
			...
			fill(vertexColor);
			ellipse(vertexLocation.x, vertexLocation.y, VERTEX_DIAMETER,
					VERTEX_DIAMETER);
			...
			if (showVertexLabel) {
				...
				if (toggleDijkstra) {
					if (minimumDistanceFromSourceVertex
						...
						text(vertexLabel + " (Infinity)", vertexLocation.x
								+ (VERTEX_DIAMETER / 2.5f), vertexLocation.y
								- (VERTEX_DIAMETER / 2.5f));
					} else {
						text(vertexLabel + " ("
					...
					text(vertexLabel + " (" + adjacencyList.size() + ")",
							vertexLocation.x + (VERTEX_DIAMETER / 2.5f),
							vertexLocation.y - (VERTEX_DIAMETER / 2.5f));
				}
			}
