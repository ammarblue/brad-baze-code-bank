 ...
 * 
 * This functionality has been split from the MD2_Model class to aid future
 * development. The following feature(s) of the MD2 model format have not been
 * implement yet: multiple skin files
 * 
 * @author Peter Lager
 * 
 */
public class MD2_Loader {
	...
	/*
	 * Each frame in the file has a name which is used to identify the different
	 * model animation sets.
	 */
	...
	private int[] glCommands; // Type (FAN or STRIP and number of vertices to
								// render
	...
	/**
	 * This method performs 3 main tasks 1) Create the model texture 2) Load the
	 * model data from file 3) Create a MD2_Model object
	 * 
	 * @param modelFilename
	 * @param texFilename
	 * @return a MD2_Model object or null if insuccessful
	 */
	public MD2_Model loadModel(String modelFilename, String texFilename) {
		...
		if (createBuffer(modelFilename) == 0) {
			...
			model = new MD2_Model(app, numFrames, numXYZ, framesize, points,
					modOffset, modSize, glCommands, glVertex, tex, states);
		} else {
	...
	private int createBuffer(String mdlFile) {
		...
		try {
			...
			buf = ByteBuffer.wrap(b); // BIG_ENDIAN is the default - might be
										// for Java
	...
	private void getPointList() {
		...
		points = new Vector3[numFrames * numXYZ];
		int index = 0;
		float x, y, z;
		...
		modOffset = new Vector3((minX + maxX) / 2, (minY + maxY) / 2,
				(minZ + maxZ) / 2);
		modSize = new Vector3(maxX - minX, maxY - minY, maxZ - minZ);
	}

	/**
	 * * Get the next 2 bytes from the buffer and interpret as * a signed
	 * integer (twos complement) * @return
	 */
	...
	/**
	 * * Get the next 2 bytes from the buffer and interpret as * an unsigned
	 * integer * @return
	 */
	...
	/**
	 * * Get the next byte from the buffer and interpret as * an unsigned
	 * integer (0 - 255) * @return
	 */
	...
	/**
	 * Extract a string from the buffer until a zero byte or nbytes have been
	 * read. The buffer pointer is advanced by nbytes irrespective of string
	 * length.
	 * 
	 * @param nbytes
	 * @return
	...
	/**
	 * Extract a string from the buffer until the stopAt character is reached
	 * (string does not include stopAt char) or until nbytes have been read. The
	 * buffer pointer is advanced by nbytes irrespective of string length.
	 * 
	 * @param nbytes
	 * @return
	 */
	...
	/**
	 * Create a list of model states based on frame names with ant trailing
	 * digits removed e.e. crouch001, crouch002 both brcome crouch.
	 */
	private void getModelStates() {
		...
		states = (MD2_ModelState[]) list
				.toArray(new MD2_ModelState[list.size()]);
	}
	 ...
	 * GL_TRIANGLE_FAN) and the vertices (index to points array * and texture
	 * coordinates)
	 */
	public void getGLcommands() {
		...
		/*
		 * Convert LinkedList<Integer> to int array of the correct size This
		 * will unbox the integers just once so should make the rendering
		 * quicker.
		 */
		...
		/*
		 * convert LinkedList<Vertex> to Vertex array of correct size. Faster to
		 * traverse array than list when we come to render
		 */
		glVertex = (GL_Vertex[]) verts.toArray(new GL_Vertex[verts.size()]);
	}

	/*
	 * The remaining methods in this class were used to confirm that the MD2
	 * file was read correctly and are not needed in the distribution version
	 */
	public void displayGLcommands() {
		...
		for (int i = 0; i < glCommands.length; i++) {
			...
			if (cmd < 0) {
				cmd = -cmd;
				System.out.println("GL_TRIANGLE_FAN    ( " + cmd
						+ " vertices )");
			} else if (cmd > 0) {
				System.out.println("GL_TRIANGLE_STRIP  ( " + cmd
						+ " vertices )");
			}
			for (int j = 0; j < cmd; j++) {
	...
	public void displayHeader() {
		System.out.println("Ident " + ident + " "); // identifies as MD2 file
													// "IDP2"
		System.out.println("Version " + version + " "); // mine is 8
		System.out.println("Skin size " + skinwidth + " x " + skinheight); // width
																			// of
																			// texture
		System.out.println("Frame size " + framesize + " bytes"); // number of
																	// bytes per
																	// frame
		System.out.println("Number of skins " + numSkins + " listed"); // number
																		// of
																		// textures
		System.out.println("Number of vertices " + numXYZ + " per frame"); // number
																			// of
																			// pos
		System.out.println("Number of textute coords " + numST + " "); // number
																		// of
																		// texture
		System.out.println("Number of triangles " + numTris + " "); // number of
																	// triangles
		System.out.println("Number of GL commands " + numGLcmds + " ");
		System.out.println("Number of frames " + numFrames + " "); // total
																	// number of
																	// frames
		System.out.println(">> Offset to skins       " + offsetSkins + " "); // offset
																				// to
																				// skin
																				// names
																				// (64
																				// bytes
																				// each)
		System.out.println(">> Offset to tex coords  " + offsetST + " "); // offset
																			// of
																			// texture
																			// s-t
																			// values
		System.out.println(">> Offset to triangles   " + offsetTris + " "); // offset
																			// of
																			// triangle
																			// mesh
		System.out.println(">> Offset to frame data  " + offsetFrames + " "); // offset
																				// of
																				// frame
																				// data
																				// (pos)
		System.out.println(">> Offset to GL commands " + offsetGLcmds + " "); // type
																				// of
																				// OpenGL
																				// commands
																				// to
																				// use
		System.out.println(">> Offset end of file    " + offsetEnd + " "); // end
																			// of
																			// file
	}
}
