/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/11113*@* */
...
public class Ogro extends PApplet{
	...
	public void setup() {
		...
		dmat = new PMatrix3D((PMatrix3D) getMatrix()); // copy the initial
														// matrix
		...
		model1 = loader.loadModel("models/ogro/ogro.md2",
				"models/ogro/ogrobase.jpg");
		model2 = loader.loadModel("models/ogro/weapon.md2",
				"models/ogro/weapon.jpg");

		loader = null; // loader no lomger required so get rid of it to release
						// RAM
		...
		/*
		 * GFont.getFont() - parameters 1) this (always) 2) font name (see
		 * below) 3) font size
		 * 
		 * The font name will depend on the OS used and fonts installed. It
		 * should be the same as those listed in the 'Create Font' tool in
		 * processing. Alternatively use println(PFont.list()); in a Processing
		 * sketch
		 */
		...
		pnl = new GPanel(this, "Model Controller (click to hide/show)", 0, 0,
				width, height - 16);
		pnl.setOpaque(false);
		...
		if (model1 != null) {
			...
			for (int i = 0; i < mdlStates.length; i++) {
				optStates[i] = new GOption(this, mdlStates[i].name, 10,
						70 + i * 18, 100);
	...
	public void pre() {
		interpolate = PApplet.constrain(3.1f / (frameRate + 0.01f), 0.001f, 0.8f);
		if (model1 != null)
