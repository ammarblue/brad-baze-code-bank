...
public class GravityGrid extends PApplet {
	...
	public void setup() {
		if (!RESET) {
			...
			cam = new PeasyCam(this, sizeX / 2, sizeY / 2, 0, 600); // default
																	// settings
																	// on double
																	// click
		...
		grid = new Grid(gridPointsX, gridPointsY,this);
		float s = 1.5f; // scale Grid
		...
		for (int i = 0; i < gF.length; i++) {
			...
			int col = color(random(200, 255), random(200, 255), random(1));
			gF[i] = new GravityField(id, x, y, radius, col,this);
		} // end for i
		...
		for (int i = 0; i < pl.length; i++) {
			...
			int col = color(random(110, 255), random(110, 255),
					random(110, 220));
			pl[i] = new Planet(id, x, y, dirX, dirY, radius, col,this);
		} // end for i
	...
	public void draw() {
		...
		for (int i = 0; i < pl.length; i++) {
			for (int j = 0; j < gF.length; j++)
				pl[i].setNewDirection(gF[j].x, gF[j].y, gF[j].radius,
						1500 * gF.length, .4f);
			for (int j = 0; j < pl.length; j++) {
				if (j != i)
					pl[i].setNewDirection(pl[j].x, pl[j].y, pl[j].radius, 2000,
							.6f);
			}
			pl[i].setNewPosition();
		} // end for i
		...
		for (int i = 0; i < gF.length; i++)
			grid.setGridPointPositionZ(gF[i].x, gF[i].y, gF[i].radius);
		for (int i = 0; i < pl.length; i++)
			grid.setGridPointPositionZ(pl[i].x, pl[i].y, pl[i].radius);

		grid.drawGrid(); // draw grid
		for (int i = 0; i < gF.length; i++)
			gF[i].drawSphere(20, 30); // draw GravityFields
		for (int i = 0; i < pl.length; i++)
			pl[i].drawSphere(10, 20); // draw Planets
		if (showGravityFieldLabel)
			for (int i = 0; i < gF.length; i++)
				gF[i].drawLabel(); // draw GravityFieldsLabel
	} // end void draw

	public void keyReleased() {
		if (key == 't') {
			if (showGravityFieldLabel)
				showGravityFieldLabel = false;
			else
				showGravityFieldLabel = true;
		}
		if (key == 'r') {
			RESET = true;
			setup();
			...
			if (key == 'm' || key == 'M') {
				...
				if (!mouseStillPressed) {
					...
					for (int i = 0; i < gF.length; i++) {
						...
						float mouseDis = sqrt(sq(mouseX - scrX)
								+ sq(mouseY - scrY));
						...
						// scrX = screenX( gF[indexGF].x, gF[indexGF].y, 0 );
						// scrY = screenY( gF[indexGF].x, gF[indexGF].y, 0 );
						// cam.beginHUD(); stroke(0,255,255); strokeWeight(10);
						// point(scrX, scrY); cam.endHUD();
				...
				for (int i = 0; i < grid.px.length; i++) {
					...
					// scrX = screenX( grid.px[indexGP], grid.py[indexGP], 0 );
					// scrY = screenY( grid.px[indexGP], grid.py[indexGP], 0 );
					// cam.beginHUD(); stroke(0,255,255); strokeWeight(20);
					// point(scrX, scrY);cam.endHUD();
				...
				if (mousePressed) {
					...
					if (mouseButton == RIGHT) {
						...
						if (gF[indexGF].radius < 10)
							gF[indexGF].radius = 10;
						if (gF[indexGF].radius > 100)
							gF[indexGF].radius = 100;
					} // end if
				} else {
