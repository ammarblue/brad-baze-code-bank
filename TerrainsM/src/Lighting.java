...
class Lighting {
	...
	Lighting(terrainmarch terrainmarch, PVector sunDirection, float sunUp, boolean uc) {
		this.terrainmarch = terrainmarch;
		useClouds = uc;
		// simple time of day stuff sunset -> sunrise

		_sc = this.terrainmarch.smoothblend(new PVector(1.f, 0.3f, 0.05f), new PVector(1.f,
				0.8f, 0.5f), sunUp);
		skytop = this.terrainmarch.smoothblend(new PVector(0.1f, 0.2f, 0.4f), new PVector(
				0.25f, 0.4f, 0.7f), sunUp);
		skybot = this.terrainmarch.smoothblend(new PVector(1.2f, 0.5f, 0.05f), new PVector(
				0.5f, 0.8f, 1.0f), sunUp);
		fogcol = this.terrainmarch.smoothblend(new PVector(1.f, 0.5f, 0.05f), new PVector(
				0.45f, 0.65f, 0.9f), sunUp);
		_am = this.terrainmarch.smoothblend(new PVector(0.5f, 0.4f, 0.3f), new PVector(0.3f,
				0.4f, 0.6f), sunUp);
		_sd = sunDirection;
		_sd.normalize();
	...
	public PVector getShading(QualitySettings qs, HieghtField hgts,
			PVector p, PVector n, PVector m, float t) {
		// ? shot shadow ray?
