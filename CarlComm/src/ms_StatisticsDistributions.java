...
public class ms_StatisticsDistributions {
	...
	/*
	 * repeated calls with variable r as a randomStream input will produce an
	 * exponential distribution with mean as the user specifies. A single random
	 * number is produced per call.
	 */
	...
	public static int Geometric(double probabilityOfSuccess, Random r) {
		...
		if (probabilityOfSuccess > 0.1) {
			do {
				n++;
			} while (r.nextDouble() >= probabilityOfSuccess);
			return n;
		}
		n = (int) (Math.log(r.nextDouble()) / Math
				.log(1.0 - probabilityOfSuccess));
		return n;
	}

	/*
	 * Input a random stream to produce random numbers and this procedure
	 * produces a normal distribution with the user specified mean and standard
	 * deviation when called repeatedly. A single random number is produced per
	 * call.
	 */
	public static double Normal(double mean, double std, Random r) {
		...
		if (rr == 0.0)
			rr = 0.0000001;
		return mean + std * Math.sqrt(Math.log(rr) * (-2))
				* Math.sin(r.nextDouble() * Math.PI * 2);
	}
	...
	/*
	 * With an input of a random stream to produce random numbers and a limit of
	 * type int, this procedure will produce a uniform distribution over the
	 * interval (0..limit) when called repeatedly. One random number per call is
	 * produced.
	 */
	...
	/*
	 * With an input of a random stream to produce random numbers and a limit of
	 * type long, this procedure will produce a uniform distribution over the
	 * interval (0..limit) when called repeatedly. One random number per call is
	 * produced.
	 */
